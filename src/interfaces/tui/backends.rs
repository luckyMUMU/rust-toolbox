...
use crate::interfaces::tui::sync::{SyncBackend, CacheBackend, CacheStats, CacheMetadata, CachePriority, CompactionResult, DataSource};
...
impl SyncBackend for MockSyncBackend {
        ...
        Ok(vec![
            ...
            PluginInfo {
                ...
                metadata: {
                    ...
                    meta.insert("dependencies".to_string(), serde_json::json!(["python>=3.8"]));
                    meta
                },
            },
        ])
    ...
    async fn fetch_system_status(&self) -> Result<SystemStatus> {
        ...
        Ok(SystemStatus {
            ...
            system_health: if rng.gen_bool(0.8) { SystemHealth::Healthy } else { SystemHealth::Warning },
...
impl MemoryCacheBackend {
    ...
    async fn evict_for_space(&self, needed_bytes: u64) {
        ...
        let mut entries_to_evict: Vec<String> = storage.iter()
        ...
        if evicted_count > 0 {
            ...
            debug!("Evicted {} cache entries to free {} bytes", evicted_count, freed_bytes);
        }
    }
}
...
impl CacheBackend for MemoryCacheBackend {
    ...
    async fn import(&self, data: &[u8]) -> Result<()> {
        let import_data: HashMap<String, CacheEntry> = serde_json::from_slice(data)
            .map_err(|e| WorkflowError::validation(e.to_string()))?;
        ...
        info!("Imported {} cache entries ({} bytes)", stats.total_entries, stats.total_size_bytes);
        Ok(())
    }
}
...
impl SyncBackend for HttpSyncBackend {
    async fn fetch_workflows(&self) -> Result<Vec<WorkflowInfo>> {
        let response = self.build_request(reqwest::Method::GET, "/api/workflows")
            .send()
            .await
            .map_err(|e| WorkflowError::validation(e.to_string()))?;
    ...
    async fn fetch_executions(&self) -> Result<Vec<crate::interfaces::tui::state::ExecutionInfo>> {
        let response = self.build_request(reqwest::Method::GET, "/api/executions")
            .send()
            .await
            .map_err(|e| WorkflowError::validation(e.to_string()))?;
        ...
        let executions: Vec<crate::interfaces::tui::state::ExecutionInfo> = response
            .json()
            .await
            .map_err(|e| WorkflowError::validation(e.to_string()))?;
    ...
    async fn fetch_tools(&self) -> Result<Vec<ToolInfo>> {
        let response = self.build_request(reqwest::Method::GET, "/api/tools")
            .send()
            .await
            .map_err(|e| WorkflowError::validation(e.to_string()))?;
    ...
    async fn fetch_plugins(&self) -> Result<Vec<PluginInfo>> {
        let response = self.build_request(reqwest::Method::GET, "/api/plugins")
            .send()
            .await
            .map_err(|e| WorkflowError::validation(e.to_string()))?;
    ...
    async fn fetch_system_status(&self) -> Result<SystemStatus> {
        let response = self.build_request(reqwest::Method::GET, "/api/system/status")
            .send()
            .await
            .map_err(|e| WorkflowError::validation(e.to_string()))?;
