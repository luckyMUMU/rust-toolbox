...
use crate::performance::memory::{MemoryManager, MemoryConfig, MemoryUsage, MemoryStatistics, PressureLevel};
...
impl TuiMemoryManager {
    ...
    pub async fn track_widget_allocation(&self, widget_id: &str, widget_type: &str, size: usize) -> Result<()> {
        let config = self.config.read().await;
        if !config.track_widget_memory {
            return Ok(());
        }
        ...
        *current_usage += size;
        ...
        let mut widget_usage = self.widget_usage.entry(widget_id.to_string()).or_insert_with(|| {
            WidgetMemoryUsage {
        ...
        *total_allocations += 1;
        ...
        self.memory_manager.record_usage(&format!("tui_widget_{}", widget_id), usage).await;
    ...
    pub async fn get_render_buffer(&self, width: u16, height: u16) -> Result<ratatui::buffer::Buffer> {
        let config = self.config.read().await;
        if !config.enable_buffer_pooling {
            return Ok(ratatui::buffer::Buffer::empty(ratatui::layout::Rect::new(0, 0, width, height)));
        }
        ...
        Ok(ratatui::buffer::Buffer::empty(ratatui::layout::Rect::new(0, 0, width, height)))
    }
    ...
    pub async fn cache_render_buffer(&self, key: &str, buffer: ratatui::buffer::Buffer) -> Result<()> {
        let config = self.config.read().await;
        if !config.enable_buffer_pooling {
            return Ok(());
        }
        ...
        self.buffer_cache.insert(key.to_string(), (buffer, Instant::now()));
        Ok(())
    }
    ...
    pub async fn get_cached_render_buffer(&self, key: &str) -> Option<ratatui::buffer::Buffer> {
        ...
        if let Some((buffer, cached_at)) = self.buffer_cache.get(key) {
    ...
    pub async fn force_cleanup(&self) -> Result<CleanupReport> {
        ...
        match self.cleanup_buffer_cache().await {
            Ok(freed_buffers) => {
                ...
                report.freed_memory += freed_buffers * std::mem::size_of::<ratatui::buffer::Buffer>();
            }
            Err(e) => {
                report.errors.push(format!("Buffer cache cleanup failed: {}", e));
        ...
        match self.cleanup_buffer_pool().await {
            ...
            Err(e) => {
                report.errors.push(format!("Buffer pool cleanup failed: {}", e));
                report.success = false;
            }
        }
        ...
        match self.cleanup_widget_tracking().await {
            ...
            Err(e) => {
                report.errors.push(format!("Widget tracking cleanup failed: {}", e));
                report.success = false;
            }
        }
        ...
        match self.memory_manager.force_cleanup().await {
            ...
            Err(e) => {
                report.errors.push(format!("Memory manager cleanup failed: {}", e));
                report.success = false;
            }
        }
    ...
    pub async fn get_memory_statistics(&self) -> Result<TuiMemoryStatistics> {
        ...
        let widget_stats: HashMap<String, WidgetMemoryUsage> = self.widget_usage
            .iter()
    ...
    async fn cleanup_buffer_cache(&self) -> Result<usize> {
        ...
        let expired_keys: Vec<String> = self.buffer_cache
            .iter()
            .filter_map(|entry| {
        ...
        if self.buffer_cache.len() > config.max_cached_buffers {
            let mut entries: Vec<(String, Instant)> = self.buffer_cache
                .iter()
    ...
    async fn cleanup_widget_tracking(&self) -> Result<Vec<String>> {
        ...
        let stale_widgets: Vec<String> = self.widget_usage
            .iter()
            .filter_map(|entry| {
...
mod tests {
    ...
    async fn test_widget_memory_tracking() {
        ...
        // Track allocation
        tui_manager.track_widget_allocation("test_widget", "TestWidget", 1024).await.unwrap();
        ...
        // Track deallocation
        tui_manager.track_widget_deallocation("test_widget", 512).await.unwrap();
    ...
    async fn test_memory_leak_detection() {
        ...
        tui_manager.track_widget_allocation("normal_widget", "NormalWidget", 1000).await.unwrap();
        
        // Simulate potential leak
        tui_manager.track_widget_allocation("leaky_widget", "LeakyWidget", 1000).await.unwrap();
        
        // Set baseline
        tui_manager.leak_detector.baseline_usage.insert("leaky_widget".to_string(), 1000);
        
        // Simulate memory growth
        tui_manager.track_widget_allocation("leaky_widget", "LeakyWidget", 2000).await.unwrap();
    ...
    async fn test_force_cleanup() {
        ...
        tui_manager.track_widget_allocation("test_widget", "TestWidget", 1024).await.unwrap();
        let buffer = tui_manager.get_render_buffer(80, 24).await.unwrap();
        tui_manager.cache_render_buffer("test_cache", buffer).await.unwrap();
