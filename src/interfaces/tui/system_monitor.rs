...
impl SystemMonitor {
    ...
    async fn collect_system_metrics(&self) -> SystemStatus {
        ...
        let thread_count = system.processes()
            .values()
    ...
    fn assess_system_health(&self, cpu_usage: f64, memory_usage: f64, disk_usage: f64) -> SystemHealth {
    ...
    pub async fn get_system_health_assessment(&self) -> SystemHealthAssessment {
        ...
        let memory_health_score = Self::calculate_memory_health_score(status.memory_usage, &memory_info);
    ...
    fn calculate_cpu_health_score(cpu_usage: f64, cpu_info: &CpuInfo) -> f64 {
        ...
        let core_adjustment: f64 = if cpu_info.core_count >= 8 { 5.0 } else if cpu_info.core_count >= 4 { 2.0 } else { 0.0 };
    ...
    fn generate_health_recommendations(
        ...
        if memory_usage >= 90.0 {
            recommendations.push("Critical: Memory usage is very high. Close unused applications or add more RAM.".to_string());
        ...
        if disk_usage >= 95.0 {
            recommendations.push("Critical: Disk space is almost full. Delete unnecessary files or expand storage.".to_string());
        } else if disk_usage >= 85.0 {
            recommendations.push("Warning: Low disk space. Clean up temporary files and consider storage expansion.".to_string());
        }
        ...
        if recommendations.is_empty() {
            recommendations.push("System is running optimally. Continue regular maintenance and monitoring.".to_string());
        }
    ...
    fn assess_system_load(load_average: f64, core_count: usize) -> String {
        ...
        match load_per_core {
            load if load >= 2.0 => "System is heavily overloaded. Performance may be severely impacted.".to_string(),
            load if load >= 1.5 => "System is overloaded. Consider reducing workload or upgrading hardware.".to_string(),
            load if load >= 1.0 => "System is at full capacity. Monitor for performance issues.".to_string(),
            load if load >= 0.7 => "System is under moderate load. Performance should be acceptable.".to_string(),
            load if load >= 0.3 => "System is under light load. Good performance expected.".to_string(),
            _ => "System is idle or under very light load. Excellent performance expected.".to_string(),
        }
    }
    ...
    pub async fn get_network_info(&self) -> Vec<NetworkInfo> {
        ...
        networks.iter().map(|(name, network)| {
            NetworkInfo {
                name: name.clone(),
    ...
    pub async fn get_top_processes(&self, limit: usize, sort_by: ProcessSortBy) -> Vec<ProcessInfo> {
        let system = self.system.read().await;
