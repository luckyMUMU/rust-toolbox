...
impl ConfirmationDialog {
    ...
    pub fn get_selected_action(&self) -> Option<&ConfirmationAction> {
        self.options.get(self.selected_option).map(|opt| &opt.action)
    }
...
pub enum FeedbackEvent {
    ...
    UpdateProgress { id: String, current: u64, message: Option<String> },
...
impl FeedbackManager {
    ...
    async fn handle_feedback_event(&self, event: FeedbackEvent) -> Result<()> {
        match event {
            ...
            FeedbackEvent::UpdateProgress { id, current, message } => {
                let mut indicators = self.progress_indicators.write().await;
    ...
    pub async fn show_progress<S: Into<String>>(&self, id: S, title: S, total: u64) -> Result<()> {
        ...
        self.feedback_sender.send(FeedbackEvent::ShowProgress(progress))
            .map_err(|e| crate::error::WorkflowError::ValidationError(format!("Failed to show progress: {}", e)))?;
        Ok(())
    }
    
    pub async fn update_progress<S: Into<String>>(&self, id: S, current: u64, message: Option<String>) -> Result<()> {
        self.feedback_sender.send(FeedbackEvent::UpdateProgress {
            id: id.into(),
            current,
            message,
        }).map_err(|e| crate::error::WorkflowError::ValidationError(format!("Failed to update progress: {}", e)))?;
        Ok(())
    }
    
    pub async fn complete_progress<S: Into<String>>(&self, id: S) -> Result<()> {
        self.feedback_sender.send(FeedbackEvent::CompleteProgress(id.into()))
            .map_err(|e| crate::error::WorkflowError::ValidationError(format!("Failed to complete progress: {}", e)))?;
        Ok(())
    }
    
    pub async fn show_notification<S: Into<String>>(&self, id: S, title: S, message: S, notification_type: NotificationType) -> Result<()> {
        let notification = Notification::new(id, title, message, notification_type);
        self.feedback_sender.send(FeedbackEvent::ShowNotification(notification))
            .map_err(|e| crate::error::WorkflowError::ValidationError(format!("Failed to show notification: {}", e)))?;
        Ok(())
    }
    
    pub async fn show_confirmation<S: Into<String>>(&self, title: S, message: S, dialog_type: DialogType) -> Result<()> {
        let dialog = ConfirmationDialog::new(title, message, dialog_type);
        self.feedback_sender.send(FeedbackEvent::ShowDialog(dialog))
            .map_err(|e| crate::error::WorkflowError::ValidationError(format!("Failed to show dialog: {}", e)))?;
        Ok(())
    }
    
    pub async fn show_status<S: Into<String>>(&self, message: S, status_type: StatusType) -> Result<()> {
        let status = StatusMessage::new(message, status_type);
        self.feedback_sender.send(FeedbackEvent::ShowStatus(status))
            .map_err(|e| crate::error::WorkflowError::ValidationError(format!("Failed to show status: {}", e)))?;
        Ok(())
    }
    
    pub async fn dismiss_dialog(&self) -> Result<()> {
        self.feedback_sender.send(FeedbackEvent::DismissDialog)
            .map_err(|e| crate::error::WorkflowError::ValidationError(format!("Failed to dismiss dialog: {}", e)))?;
        Ok(())
    }
...
impl FeedbackWidget {
    ...
    fn render_progress_indicators(&self, frame: &mut Frame, area: Rect, indicators: &[ProgressIndicator]) {
        let chunks = Layout::default()
            .direction(Direction::Vertical)
        ...
        for (i, progress) in indicators.iter().take(5).enumerate() {
            ...
            let label_text = if progress.message.is_empty() {
                ...
                format!("{} {} - {}", progress.status.icon(), progress.title, progress.message)
            };
            
            let label = Paragraph::new(label_text)
                .style(Style::default().fg(progress.status.color()));
    ...
    fn render_confirmation_dialog(&self, frame: &mut Frame, area: Rect, dialog: &ConfirmationDialog) {
        ...
        let options_paragraph = Paragraph::new(Line::from(option_text))
            .alignment(Alignment::Center);
...
impl FeedbackManager {
    ...
    pub async fn show_success<S: Into<String>>(&self, title: S, message: S) -> Result<()> {
        self.show_notification("success", title, message, NotificationType::Success).await
    }
    
    pub async fn show_warning<S: Into<String>>(&self, title: S, message: S) -> Result<()> {
        self.show_notification("warning", title, message, NotificationType::Warning).await
    }
    
    pub async fn show_error<S: Into<String>>(&self, title: S, message: S) -> Result<()> {
        self.show_notification("error", title, message, NotificationType::Error).await
    }
    
    pub async fn confirm_action<S: Into<String>>(&self, title: S, message: S) -> Result<()> {
        self.show_confirmation(title, message, DialogType::Confirmation).await
    }
