...
use ratatui::{
    layout::Rect,
    widgets::{List, ListItem, ListState, Table, TableState, Block, Borders, Scrollbar, ScrollbarState, ScrollbarOrientation},
...
pub trait VirtualDataProvider<T>: Send + Sync {
    ...
    async fn filter(&self, predicate: Box<dyn Fn(&T) -> bool + Send + Sync>, start: usize, count: usize) -> Result<Vec<(usize, T)>> {
    ...
    T: Clone + Send + Sync + 'static,
{
    ...
    pub async fn initialize(&mut self) -> Result<()> {
        ...
        tracing::debug!("Virtual list initialized with {} total items", self.total_items);
        Ok(())
    }
    ...
    pub fn update_viewport_size(&mut self, area: Rect) {
        ...
        // Update scrollbar
        self.scrollbar_state = self.scrollbar_state.viewport_content_length(available_height);
    }
    ...
    pub async fn set_search_query(&mut self, query: Option<String>) -> Result<()> {
        ...
        if let Some(ref query) = self.search_query {
            // Perform search
            let search_results = self.data_provider
            ...
            self.scrollbar_state = self.scrollbar_state.content_length(self.filtered_indices.len());
        } else {
    ...
    pub async fn render<F>(&mut self, frame: &mut Frame, area: Rect, item_renderer: F) -> Result<()>
    where
        F: Fn(&T, bool, &Style) -> ListItem + Send + Sync,
    {
        ...
        if let Some(animation) = &self.scroll_animation {
            if animation.start_time.elapsed() >= animation.duration {
                ...
                let progress = animation.start_time.elapsed().as_secs_f64() / animation.duration.as_secs_f64();
        ...
        for i in 0..self.viewport_size.min(effective_total.saturating_sub(self.viewport_start)) {
            let item_index = if self.filter_active {
                self.filtered_indices.get(self.viewport_start + i).copied().unwrap_or(0)
            } else {
            ...
            if let Some(virtual_item) = self.item_cache.get(&item_index) {
                if let Some(ref data) = virtual_item.data {
                    ...
                    list_items.push(ListItem::new("Loading...").style(Style::default().fg(Color::Yellow)));
                } else if let Some(ref error) = virtual_item.error {
                    list_items.push(ListItem::new(format!("Error: {}", error)).style(Style::default().fg(Color::Red)));
                } else {
                    list_items.push(ListItem::new("No data").style(Style::default().fg(Color::Gray)));
                }
            } else {
                list_items.push(ListItem::new("Loading...").style(Style::default().fg(Color::Yellow)));
            }
        ...
        let title = if let Some(ref query) = self.search_query {
            format!("Search: {} ({} results)", query, self.filtered_indices.len())
        } else {
        ...
        match key.code {
            ...
            KeyCode::Down => {
                if let Some(selected) = self.list_state.selected() {
                    ...
                    if selected < self.viewport_size - 1 && self.viewport_start + selected + 1 < effective_total {
                        self.list_state.select(Some(selected + 1));
            ...
            KeyCode::End => {
                ...
                if effective_total > 0 {
                    self.scroll_to(effective_total.saturating_sub(self.viewport_size)).await?;
                    self.list_state.select(Some(self.viewport_size.min(effective_total) - 1));
                }
                Ok(true)
            }
            _ => Ok(false)
    ...
    pub fn get_selected_item(&self) -> Option<&T> {
        if let Some(selected) = self.list_state.selected() {
            let item_index = if self.filter_active {
                self.filtered_indices.get(self.viewport_start + selected).copied()?
            } else {
    ...
    async fn ensure_viewport_loaded(&mut self) -> Result<()> {
        let load_start = self.viewport_start.saturating_sub(self.config.prefetch_size);
        let load_end = (self.viewport_start + self.viewport_size + self.config.prefetch_size)
            .min(if self.filter_active { self.filtered_indices.len() } else { self.total_items });
        ...
        for i in load_start..load_end {
            ...
            let needs_loading = !self.item_cache.contains_key(&item_index) ||
                self.item_cache.get(&item_index).unwrap().is_expired(self.config.cache_ttl);
    ...
    async fn load_range(&mut self, start: usize, count: usize) -> Result<()> {
        ...
        for i in start..start + count {
            ...
            let mut item = self.item_cache.entry(item_index).or_insert_with(|| VirtualItem::new(item_index));
            item.loading = true;
            item.error = None;
        }
        ...
        match self.data_provider.load_range(start, count).await {
            Ok(data) => {
                ...
                for (i, item_data) in data.into_iter().enumerate() {
                    ...
                    let height = self.data_provider.item_height(item_index).await.unwrap_or(1);
    ...
    T: Clone + Send + Sync + 'static,
{
    ...
    pub async fn render<F>(&mut self, frame: &mut Frame, area: Rect, row_renderer: F) -> Result<()>
    where
        F: Fn(&T, usize) -> Vec<String> + Send + Sync,
    {
        ...
        self.list_widget.render(frame, area, |data, is_selected, style| {
...
impl VirtualDataProvider<String> for MockDataProvider {
    ...
    async fn search(&self, query: &str, start: usize, count: usize) -> Result<Vec<(usize, String)>> {
...
mod tests {
    ...
    async fn test_search_functionality() {
        ...
        // Test search
        list.set_search_query(Some("Item 1".to_string())).await.unwrap();
