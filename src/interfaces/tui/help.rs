...
impl HelpSystem {
    ...
    fn setup_view_shortcuts(&mut self) {
        ...
        for view in [
            ...
            self.view_shortcuts.insert(view, navigation_shortcuts.clone());
        }
        
        // Add view-specific shortcuts
        self.view_shortcuts.get_mut(&ViewType::WorkflowList).unwrap().extend(vec![
            ShortcutInfo {
                key: "Enter".to_string(),
        ...
        self.view_shortcuts.get_mut(&ViewType::ExecutionMonitor).unwrap().extend(vec![
            ShortcutInfo {
                key: "Space".to_string(),
        ...
        self.view_shortcuts.get_mut(&ViewType::LogViewer).unwrap().extend(vec![
            ShortcutInfo {
                key: "1-5".to_string(),
    ...
    fn setup_default_help_content(&mut self) {
        ...
        self.context_help.insert(
            ...
            HelpContent {
                ...
                content: "所有可用的键盘快捷键按类别组织显示。快捷键会根据当前上下文动态更新。".to_string(),
                tips: vec![
    ...
    pub fn update_animation(&mut self) {
        ...
        if let Some(start_time) = self.animation_state.start_time {
            ...
            self.animation_state.progress = (elapsed.as_millis() as f32 / self.animation_state.duration.as_millis() as f32).min(1.0);
    ...
    pub fn get_current_shortcuts(&self, view: &ViewType, widget: Option<&WidgetId>) -> Vec<ShortcutInfo> {
    ...
    pub fn add_shortcut(&mut self, shortcut: ShortcutInfo, scope: ShortcutScope) {
        match scope {
            ...
            ShortcutScope::View(view) => {
                self.view_shortcuts.entry(view).or_insert_with(Vec::new).push(shortcut);
            }
            ShortcutScope::Widget(widget_id) => {
                self.widget_shortcuts.entry(widget_id).or_insert_with(Vec::new).push(shortcut);
            }
        }
    }
    ...
    fn render_shortcuts_content(&self, frame: &mut Frame, area: Rect, theme: &Theme) {
        ...
        let shortcuts_text = vec![
            Line::from(vec![
                Span::styled("全局快捷键:", theme.styles.text_bold),
            ]),
            Line::from(""),
            Line::from(vec![
            ...
            Line::from(""),
            Line::from(vec![
                Span::styled("导航快捷键:", theme.styles.text_bold),
            ]),
            Line::from(""),
            Line::from(vec![
            ...
            Line::from(""),
            Line::from(vec![
                Span::styled("视图切换:", theme.styles.text_bold),
            ]),
            Line::from(""),
            Line::from(vec![
    ...
    pub fn handle_key_event(&mut self, key: ratatui::crossterm::event::KeyEvent) -> Result<Option<Action>> {
