...
impl TuiConfigManager {
    ...
    fn load_from_file(path: &Path) -> Result<TuiConfig> {
        ...
        let config: TuiConfig = toml::from_str(&content)
            .map_err(|e| WorkflowError::Config(
                config::ConfigError::Message(format!("Failed to parse TUI config: {}", e))
            ))?;
        Ok(config)
    }
    ...
    fn validate_config(&self, config: &TuiConfig) -> Result<()> {
        ...
        if config.user_preferences.notification_settings.notification_duration == 0 {
            return Err(WorkflowError::ValidationError(
...
impl ConfigPresetManager {
    ...
    pub fn save_presets(&self, path: &Path) -> Result<()> {
        let content = toml::to_string_pretty(&self.presets)
            .map_err(|e| WorkflowError::Generic(e.into()))?;
        std::fs::write(path, content)?;
        Ok(())
    }
    ...
    pub fn load_presets(&mut self, path: &Path) -> Result<()> {
        if path.exists() {
            let content = std::fs::read_to_string(path)?;
            let presets: HashMap<String, TuiConfig> = toml::from_str(&content)
                .map_err(|e| WorkflowError::Config(
                    config::ConfigError::Message(format!("Failed to parse presets: {}", e))
                ))?;
