...
impl RenderingPerformanceManager {
    ...
    pub async fn start_frame_monitoring(&self) -> PerformanceMonitor {
        self.performance_manager.start_monitoring("tui_rendering").await
    }
    ...
    pub async fn record_frame_time(&self, render_time: Duration) -> Result<()> {
        ...
        if !frame_times.is_empty() {
            ...
            stats.average_fps = (stats.average_fps * (stats.frame_count - 1) as f64 + stats.current_fps) / stats.frame_count as f64;
    ...
    pub async fn get_cached_render(&self, widget_id: &str, area: Rect, content_hash: u64) -> Option<Buffer> {
        let config = self.config.read().await;
        if !config.enable_cache {
            return None;
        }
        
        let cache_key = format!("{}:{}:{}:{}:{}", widget_id, area.x, area.y, area.width, area.height);
    ...
    pub async fn cache_render(&self, widget_id: &str, area: Rect, content: Buffer, content_hash: u64) -> Result<()> {
        let config = self.config.read().await;
        if !config.enable_cache {
            return Ok(());
        }
    ...
    async fn evict_cache_entries(&self) {
        ...
        let mut entries: Vec<(String, Instant, u64)> = self.render_cache
            .iter()
    ...
    pub async fn should_use_incremental_rendering(&self, widget_id: &str, content_hash: u64) -> bool {
        let config = self.config.read().await;
...
mod tests {
    ...
    async fn test_frame_rate_recording() {
        ...
        render_manager.record_frame_time(Duration::from_millis(16)).await.unwrap(); // ~60 FPS
        render_manager.record_frame_time(Duration::from_millis(17)).await.unwrap();
        render_manager.record_frame_time(Duration::from_millis(15)).await.unwrap();
    ...
    async fn test_render_caching() {
        ...
        let cached = render_manager.get_cached_render("test_widget", area, content_hash).await;
        assert!(cached.is_none());
        
        // Cache the content
        render_manager.cache_render("test_widget", area, buffer.clone(), content_hash).await.unwrap();
    ...
    async fn test_dirty_region_tracking() {
        ...
        render_manager.mark_dirty("widget1".to_string(), area1, 1).await.unwrap();
        render_manager.mark_dirty("widget1".to_string(), area2, 2).await.unwrap();
    ...
    async fn test_incremental_rendering() {
        ...
        let should_render = render_manager.should_use_incremental_rendering("test_widget", content_hash).await;
        ...
        let should_render = render_manager.should_use_incremental_rendering("test_widget", 54321u64).await;
        assert!(should_render);
    }
    ...
    async fn test_optimization_report() {
        ...
        // Simulate low FPS
        for _ in 0..10 {
            render_manager.record_frame_time(Duration::from_millis(50)).await.unwrap(); // ~20 FPS
        }
        ...
        let has_complexity_recommendation = report.recommendations.iter()
            .any(|r| matches!(r.recommendation_type, RenderingOptimizationType::ReduceRenderComplexity));
