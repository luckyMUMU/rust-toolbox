...
use crate::plugins::types::{PluginConfig, PluginStatus};
...
impl PluginManagerWidget {
    ...
    fn apply_filter_and_sort(&mut self) {
        ...
        self.filtered_plugins.sort_by(|&a, &b| {
            ...
            match self.sort_order {
                ...
                PluginSortOrder::TypeAsc => {
                    plugin_a.info.plugin_type.cmp(&plugin_b.info.plugin_type)
                }
                PluginSortOrder::TypeDesc => {
                    plugin_b.info.plugin_type.cmp(&plugin_a.info.plugin_type)
                }
                PluginSortOrder::LastUpdatedAsc => {
                    plugin_a.last_updated.cmp(&plugin_b.last_updated)
                }
                PluginSortOrder::LastUpdatedDesc => {
                    plugin_b.last_updated.cmp(&plugin_a.last_updated)
                }
            }
        });
    }
    ...
    fn matches_filter(&self, plugin: &PluginDisplayInfo) -> bool {
        ...
        if !self.filter.search_term.is_empty() {
            ...
            let matches_description = plugin
                .info
                .description
                .as_ref()
    ...
    fn update_selection(&mut self) {
        ...
        self.scroll_state = self
            .scroll_state
            .content_length(self.filtered_plugins.len());
            ...
            let memory_check = if plugin.resource_usage.memory_mb > 1024.0 {
                ...
                HealthCheckItem {
                    ...
                    message: format!(
                        ...
                        plugin.resource_usage.memory_mb
                    ),
                    severity: HealthCheckSeverity::Medium,
                }
            } else {
    ...
    pub async fn install_plugin(
        ...
        if plugin_name.contains("invalid") {
            self.set_plugin_installation_status(
                plugin_name,
                PluginInstallationStatus::Failed("Invalid plugin".to_string()),
            );
    ...
    pub async fn disable_plugin(&mut self, plugin_name: &str) -> Result<(), String> {
        ...
        if !dependents.is_empty() {
            return Err(format!(
                "Cannot disable plugin '{}': required by {}",
                plugin_name,
                dependents.join(", ")
            ));
        }
    ...
    pub async fn update_plugin(&mut self, plugin_name: &str) -> Result<(), String> {
        ...
        if let Some(plugin) = self.plugins.iter_mut().find(|p| p.info.name == plugin_name) {
            ...
            tracing::info!(
                "Successfully updated plugin '{}' to version {}",
                plugin_name,
                new_version
            );
            Ok(())
    ...
    fn set_plugin_installation_status(
        &mut self,
        plugin_name: &str,
    ...
    pub fn analyze_dependencies(&self) -> DependencyResolution {
                ...
                if let Some(dep_plugin) = self.plugins.iter().find(|p| p.info.name == *dep) {
                    ...
                    if plugin.info.version != dep_plugin.info.version {
                        warnings.push(format!(
                            "Version mismatch: '{}' expects '{}' but '{}' is installed",
        ...
        let can_proceed = conflicts
            .iter()
            .all(|c| c.conflict_type != ConflictType::CircularDependency);
    ...
    pub fn resolve_batch_dependencies(
        &self,
        plugin_names: &[String],
        operation: &str,
    ) -> DependencyResolution {
        ...
        self.market_plugins = vec![
            PluginInfo {
                ...
                metadata: {
                    ...
                    meta.insert(
                        "category".to_string(),
                        serde_json::Value::String("text-processing".to_string()),
                    );
                    meta.insert(
                        "rating".to_string(),
                        serde_json::Value::String("4.8".to_string()),
                    );
                    meta.insert(
                        "downloads".to_string(),
                        serde_json::Value::String("15420".to_string()),
                    );
                    meta.insert(
                        "homepage".to_string(),
                        serde_json::Value::String(
                            "https://github.com/community/text-processor".to_string(),
                        ),
                    );
                    meta.insert(
                        "license".to_string(),
            ...
            PluginInfo {
                ...
                metadata: {
                    ...
                    meta.insert(
                        "rating".to_string(),
                        serde_json::Value::String("4.6".to_string()),
                    );
                    meta.insert(
                        "downloads".to_string(),
                        serde_json::Value::String("8932".to_string()),
                    );
                    meta.insert(
                        "homepage".to_string(),
                        serde_json::Value::String("https://datacorp.com/analyzer".to_string()),
                    );
                    meta.insert(
                        "license".to_string(),
                        serde_json::Value::String("Apache-2.0".to_string()),
                    );
                    meta
                },
            },
            PluginInfo {
                ...
                description: Some(
                    "Security vulnerability scanner and compliance checker".to_string(),
                ),
                metadata: {
                    let mut meta = std::collections::HashMap::new();
                    meta.insert(
                    ...
                    meta.insert(
                        "rating".to_string(),
                        serde_json::Value::String("4.9".to_string()),
                    );
                    meta.insert(
                        "downloads".to_string(),
                        serde_json::Value::String("23156".to_string()),
                    );
                    meta.insert(
                        "homepage".to_string(),
                        serde_json::Value::String("https://secureteam.io/scanner".to_string()),
                    );
                    meta.insert(
                        "license".to_string(),
                        serde_json::Value::String("GPL-3.0".to_string()),
                    );
                    meta
                },
            },
            PluginInfo {
                ...
                description: Some(
                    "AI-powered workflow optimization and performance tuning".to_string(),
                ),
                metadata: {
                    let mut meta = std::collections::HashMap::new();
                    meta.insert(
                    ...
                    meta.insert(
                        "rating".to_string(),
                        serde_json::Value::String("4.7".to_string()),
                    );
                    meta.insert(
                        "downloads".to_string(),
                    ...
                    meta.insert(
                        "homepage".to_string(),
                        serde_json::Value::String("https://optimize.corp/workflow".to_string()),
                    );
                    meta.insert(
                        "license".to_string(),
                        serde_json::Value::String("Commercial".to_string()),
                    );
                    meta
                },
            },
            PluginInfo {
                ...
                description: Some(
                    "Universal API connector with authentication and rate limiting".to_string(),
                ),
                metadata: {
                    let mut meta = std::collections::HashMap::new();
                    meta.insert(
                        "category".to_string(),
                        serde_json::Value::String("integration".to_string()),
                    );
                    meta.insert(
                        "rating".to_string(),
                        serde_json::Value::String("4.5".to_string()),
                    );
                    meta.insert(
                        "downloads".to_string(),
                        serde_json::Value::String("12890".to_string()),
                    );
                    meta.insert(
                        "homepage".to_string(),
                        serde_json::Value::String(
                            "https://github.com/apiteam/connector".to_string(),
                        ),
                    );
                    meta.insert(
                        "license".to_string(),
                        serde_json::Value::String("BSD-3-Clause".to_string()),
                    );
                    meta
                },
            },
    ...
    pub fn filter_market_plugins(
        &self,
        category: Option<&str>,
        plugin_type: Option<crate::core::PluginType>,
    ) -> Vec<&PluginInfo> {
        self.market_plugins
            .iter()
            .filter(|plugin| {
    ...
    pub async fn install_from_market(&mut self, plugin_name: &str) -> Result<(), String> {
        ...
        self.set_plugin_installation_status(plugin_name, PluginInstallationStatus::Completed);
        tracing::info!(
            "Successfully installed plugin '{}' from market",
            plugin_name
        );
        Ok(())
    }
    ...
    pub async fn verify_plugin_package(
        &self,
        plugin_name: &str,
        package_url: &str,
    ) -> Result<bool, String> {
        tracing::info!(
            "Verifying plugin package: {} from {}",
            plugin_name,
            package_url
        );
    ...
    async fn perform_status_monitoring(&mut self) {
        ...
        let plugin_updates: Vec<(String, PluginStatus, Option<String>, ResourceUsage)> = self
            .plugins
            .iter()
            ...
            .map(|plugin| {
                ...
                let (new_status, reason) =
                    if current_status == PluginStatus::Running && rand::random::<f64>() < 0.01 {
                        ...
                        (
                            ...
                            Some("Recovered from error".to_string()),
                        )
                    } else {
                        (current_status, None)
                    };
                ...
                new_usage.cpu_percent = (new_usage.cpu_percent
                    + (rand::random::<f64>() - 0.5) * 10.0)
                    .max(0.0)
                    .min(100.0);
                new_usage.memory_mb =
                    (new_usage.memory_mb + (rand::random::<f64>() - 0.5) * 50.0).max(0.0);
...
impl PluginManagerWidget {
    ...
    async fn handle_list_view_keys(
        &mut self,
        key: KeyEvent,
    ) -> std::result::Result<Option<Action>, WidgetError> {
        match key.code {
    ...
    async fn handle_details_view_keys(
        &mut self,
        key: KeyEvent,
    ) -> std::result::Result<Option<Action>, WidgetError> {
        match key.code {
            ...
            KeyCode::Char('U') => {
                // Update plugin
                if let Some(plugin) = self.selected_plugin() {
                    return Ok(Some(Action::Custom(
                        "UpdatePlugin".to_string(),
            ...
            KeyCode::Char('H') => {
                ...
                if let Some(plugin) = self.selected_plugin() {
                    ...
                    tracing::info!(
                        "Plugin '{}' has {} health check records",
                        plugin.info.name,
                        history.len()
                    );
                    return Ok(Some(Action::Custom(
                        "ShowHealthHistory".to_string(),
            ...
            KeyCode::Char('S') => {
                ...
                if let Some(plugin) = self.selected_plugin() {
                    ...
                    tracing::info!(
                        "Plugin '{}' has {} status change records",
                        plugin.info.name,
                        history.len()
                    );
                    return Ok(Some(Action::Custom(
                        "ShowStatusHistory".to_string(),
                        serde_json::json!({"plugin_name": plugin.info.name, "history_count": history.len()}),
                    )));
                }
            }
            ...
            KeyCode::Char('P') => {
                ...
                if let Some(plugin) = self.selected_plugin() {
                    return Ok(Some(Action::Custom(
                        "ShowPluginPermissions".to_string(),
                        serde_json::json!({"plugin_name": plugin.info.name}),
                    )));
                }
            }
    ...
    async fn handle_market_view_keys(
        &mut self,
        key: KeyEvent,
    ) -> std::result::Result<Option<Action>, WidgetError> {
        match key.code {
            ...
            KeyCode::Char('F') => {
                // Filter by category
                return Ok(Some(Action::Custom(
                    "FilterMarketByCategory".to_string(),
            ...
            KeyCode::Char('D') => {
                ...
                if let Some(plugin) = self.market_plugins.get(self.market_selected) {
                    return Ok(Some(Action::Custom(
                        "ShowMarketPluginDependencies".to_string(),
    ...
    async fn handle_dependencies_view_keys(
        &mut self,
        key: KeyEvent,
    ) -> std::result::Result<Option<Action>, WidgetError> {
        match key.code {
            ...
            KeyCode::Char('a') => {
                ...
                tracing::info!(
                    "Dependency analysis: {} conflicts, {} warnings, can_proceed: {}",
                    analysis.conflicts.len(),
                    analysis.warnings.len(),
                    analysis.can_proceed
                );
                return Ok(Some(Action::Custom(
                    "AnalyzeDependencies".to_string(),
                    serde_json::json!({
            ...
            KeyCode::Char('C') => {
                ...
                if !analysis.conflicts.is_empty() {
                    return Ok(Some(Action::Custom(
                        "ShowDependencyConflicts".to_string(),
                        serde_json::json!({
                            "conflicts": analysis.conflicts.len()
                        }),
                    )));
            ...
            KeyCode::Char('R') => {
                ...
                if let Some(plugin) = self.selected_plugin() {
                    let resolution =
                        self.resolve_batch_dependencies(&[plugin.info.name.clone()], "install");
                    return Ok(Some(Action::Custom(
                        "ResolveDependencies".to_string(),
                        serde_json::json!({
            ...
            KeyCode::Char('I') => {
                ...
                let analysis = self.analyze_dependencies();
                return Ok(Some(Action::Custom(
                    "ShowInstallationOrder".to_string(),
                    serde_json::json!({
...
impl PluginManagerWidget {
    ...
    fn create_plugin_list_item<'a>(
        &self,
        plugin: &'a PluginDisplayInfo,
        _theme: &Theme,
    ) -> ListItem<'a> {
        ...
        let status_style = match plugin.status {
            ...
            PluginStatus::Initializing | PluginStatus::ShuttingDown => {
        ...
        let enabled_str = plugin
            .config
            .as_ref()
            .map(|c| if c.enabled { "启用" } else { "禁用" })
            .unwrap_or("未知");

        let monitoring_str = if plugin.monitoring_enabled {
            "监控"
        } else {
            "无监控"
        };
        ...
        let content = vec![
            ...
            Line::from(vec![
                ...
                Span::raw(" | 工具: "),
                Span::styled(
                    plugin.tools_count.to_string(),
                    Style::default().fg(Color::Yellow),
                ),
                Span::raw(" | 内存: "),
                Span::styled(
                    format!("{:.1}MB", plugin.resource_usage.memory_mb),
    ...
    fn render_plugin_resource_usage(
        &self,
        frame: &mut Frame,
        area: Rect,
        plugin: &PluginDisplayInfo,
        ...
        let network_paragraph = Paragraph::new(network_text).style(theme.styles.info);
        frame.render_widget(network_paragraph, chunks[2]);

        // File handles
        let files_text = format!("文件句柄: {}", plugin.resource_usage.file_handles);
        let files_paragraph = Paragraph::new(files_text).style(theme.styles.info);
    ...
    fn render_plugin_description(
        ...
        let description = plugin.info.description.as_deref().unwrap_or("无描述信息");
    ...
    fn render_detailed_content(
        ...
        if let Some(config) = &plugin.config {
            ...
            content_lines.push(format!(
                "    网络访问: {}",
                if config.security_policy.allow_network_access {
                    "允许"
                } else {
                    "禁止"
                }
        ...
        content_lines.push(format!(
            "  运行时间: {}秒",
            plugin.resource_usage.uptime_seconds
        ));
        ...
        content_lines.push(format!(
            "    CPU: {:.1}%",
            plugin.resource_usage.cpu_percent
        ));
        content_lines.push(format!(
            "    内存: {:.1}MB",
            plugin.resource_usage.memory_mb
        ...
        content_lines.push(format!(
            "    网络连接: {}",
            plugin.resource_usage.network_connections
        ));
        content_lines.push(format!(
            "    文件句柄: {}",
            plugin.resource_usage.file_handles
        ));
        content_lines.push(format!(
            "    磁盘读取: {:.1}MB",
            plugin.resource_usage.disk_io_read_mb
        ));
        content_lines.push(format!(
            "    磁盘写入: {:.1}MB",
            plugin.resource_usage.disk_io_write_mb
        ));
        ...
        if self.market_plugins.is_empty() {
                ...
                .map(|(i, plugin)| {
                    ...
                    let content = vec![
                        ...
                        Line::from(vec![
                            ...
                            Span::raw(" | 作者: "),
                            Span::styled(
                                plugin.author.as_deref().unwrap_or("未知"),
            ...
            let list = List::new(items)
                .block(
                    Block::default()
                        .borders(Borders::ALL)
                        .title(format!("插件市场 ({} 个插件)", self.market_plugins.len())),
                )
                .highlight_style(theme.styles.list_item_selected)
                .highlight_symbol("► ");
    ...
    fn render_status_bar(&self, frame: &mut Frame, area: Rect, theme: &Theme) {
        ...
        // Monitoring status
        let monitoring_status = if self.monitoring_enabled {
            "开启"
        } else {
            "关闭"
        };
        ...
        let health_status = if self.health_check_enabled {
            "开启"
        } else {
            "关闭"
        };
