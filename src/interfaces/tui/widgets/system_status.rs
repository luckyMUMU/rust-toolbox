...
use crate::core::{SystemStatus, SystemHealth, NetworkStatus, SystemHealthAssessment};
use super::super::action::Action;
use super::super::theme::Theme;
use super::super::system_monitor::{SystemMonitor, CpuInfo, MemoryInfo, DiskInfo, ProcessInfo, ProcessSortBy};
use super::super::widget::{
    Widget, WidgetContext, WidgetId, WidgetCapabilities, SizeConstraints, 
    UpdateFrequency, WidgetError
};
...
impl SystemStatusWidget {
    ...
    pub fn get_maintenance_recommendations(&self) -> Vec<MaintenanceRecommendation> {
        ...
        if self.current_status.uptime.as_secs() > 7 * 24 * 3600 {
            recommendations.push(MaintenanceRecommendation {
                ...
                description: format!("系统已运行 {}，建议考虑重启以应用更新和清理内存", 
        ...
        if self.current_status.memory_usage > 85.0 {
            recommendations.push(MaintenanceRecommendation {
                ...
                description: format!("内存使用率达到 {:.1}%，建议清理内存或关闭不必要的程序", 
        ...
        if self.current_status.disk_usage > 90.0 {
            recommendations.push(MaintenanceRecommendation {
                ...
                description: format!("磁盘使用率达到 {:.1}%，建议清理临时文件和日志", 
        ...
        if self.current_status.process_count > 500 {
            recommendations.push(MaintenanceRecommendation {
                ...
                description: format!("系统运行 {} 个进程，可能影响性能", self.current_status.process_count),
    ...
    pub async fn perform_maintenance(&mut self, action: MaintenanceAction) -> Result<String, String> {
        match action {
            ...
            MaintenanceAction::Restart => {
                Err("系统重启需要管理员权限，请手动执行".to_string())
            }
    ...
    pub fn get_system_diagnostics(&self) -> SystemDiagnostics {
        ...
        if self.current_status.disk_usage > 95.0 {
            issues.push(format!("磁盘空间严重不足: {:.1}%", self.current_status.disk_usage));
        }
        
        if self.current_status.memory_usage > 95.0 {
            issues.push(format!("内存使用率过高: {:.1}%", self.current_status.memory_usage));
        }
        
        // Check warnings
        if self.current_status.cpu_usage > 80.0 {
            warnings.push(format!("CPU使用率较高: {:.1}%", self.current_status.cpu_usage));
        }
        
        if self.current_status.memory_usage > 80.0 {
            warnings.push(format!("内存使用率较高: {:.1}%", self.current_status.memory_usage));
        }
        
        if self.current_status.disk_usage > 85.0 {
            warnings.push(format!("磁盘使用率较高: {:.1}%", self.current_status.disk_usage));
        }
        
        if matches!(self.current_status.network_status, NetworkStatus::Disconnected) {
            warnings.push("网络连接断开".to_string());
        }
        
        // Add informational items
        info.push(format!("系统运行时间: {}", Self::format_duration(self.current_status.uptime)));
        ...
        if let Some(ref cpu_info) = self.cpu_info {
            info.push(format!("CPU: {} ({} 核心)", cpu_info.name, cpu_info.core_count));
        }
    ...
    pub async fn refresh_detailed_info(&mut self) {
        ...
        // Get top processes
        self.top_processes = self.system_monitor.get_top_processes(10, ProcessSortBy::Cpu).await;
    ...
    fn check_and_add_alerts(&mut self, status: &SystemStatus) {
        ...
        if matches!(status.network_status, NetworkStatus::Disconnected) {
            self.add_alert(AlertSeverity::Warning,
                ...
                "Network".to_string(), now);
        }
    }
    
    /// Add a new alert
    fn add_alert(&mut self, severity: AlertSeverity, message: String, category: String, timestamp: DateTime<Utc>) {
    ...
    fn render_resource_usage(&self, frame: &mut Frame, area: Rect, theme: &Theme) {
        ...
        let memory_gauge = Gauge::default()
            .block(Block::default()
                ...
                .title("内存使用率")
                .border_style(theme.styles.widget_border))
            .gauge_style(Style::default().fg(Self::get_usage_color(self.current_status.memory_usage)))
        ...
        let left_content = vec![
            Line::from(vec![
                ...
                Span::styled(
                    ...
                    Style::default().fg(Self::get_health_color(&self.current_status.system_health))
    ...
    fn render_recent_alerts(&mut self, frame: &mut Frame, area: Rect, theme: &Theme) {
        let recent_alerts: Vec<ListItem> = self.alerts
            .iter()
    ...
    fn handle_navigation(&mut self, key: KeyEvent) -> Option<Action> {
        match key.code {
            ...
            KeyCode::Char('r') | KeyCode::F(5) => {
                Some(Action::RefreshSystemStatus)
            }
...
impl Widget for SystemStatusWidget {
    ...
    async fn render(&mut self, frame: &mut Frame, area: Rect, theme: &Theme) -> Result<(), WidgetError> {
        let title = format!("系统状态监控 - {}", match self.display_mode {
...
impl SystemStatusWidget {
    ...
    fn render_help_overlay(&self, frame: &mut Frame, area: Rect, theme: &Theme) {
        ...
        let help_items: Vec<Line> = self.help_text()
            .iter()
            .map(|(key, desc)| {
                Line::from(vec![
                    Span::styled(*key, Style::default().fg(theme.colors.accent).add_modifier(Modifier::BOLD)),
                    Span::raw(": "),
                    Span::raw(*desc),
                ])
            })
    ...
    fn render_cpu_details(&self, frame: &mut Frame, area: Rect, theme: &Theme) {
        let content = if let Some(ref cpu_info) = self.cpu_info {
            vec![
                Line::from(vec![
                    Span::raw("CPU型号: "),
                    Span::styled(&cpu_info.name, Style::default().fg(theme.colors.text_primary)),
                ]),
                Line::from(vec![
                    Span::raw("核心数: "),
                    Span::styled(cpu_info.core_count.to_string(), Style::default().fg(theme.colors.accent)),
                ]),
                Line::from(vec![
                    Span::raw("频率: "),
                    Span::styled(format!("{} MHz", cpu_info.frequency), Style::default().fg(theme.colors.text_primary)),
                ]),
                Line::from(vec![
                    Span::raw("总体使用率: "),
                    Span::styled(
    ...
    fn render_memory_details(&self, frame: &mut Frame, area: Rect, theme: &Theme) {
        let content = if let Some(ref mem_info) = self.memory_info {
            vec![
                Line::from(vec![
                    Span::raw("总内存: "),
                    Span::styled(Self::format_bytes(mem_info.total), Style::default().fg(theme.colors.text_primary)),
                ]),
                Line::from(vec![
                    Span::raw("已使用: "),
                    Span::styled(Self::format_bytes(mem_info.used), Style::default().fg(theme.colors.accent)),
                ]),
                Line::from(vec![
                    Span::raw("可用: "),
                    Span::styled(Self::format_bytes(mem_info.available), Style::default().fg(theme.colors.success)),
                ]),
                Line::from(vec![
                    Span::raw("空闲: "),
                    Span::styled(Self::format_bytes(mem_info.free), Style::default().fg(theme.colors.text_primary)),
                ]),
                Line::from(vec![
                    Span::raw("交换区: "),
                    Span::styled(
    ...
    fn render_disk_info(&self, frame: &mut Frame, area: Rect, theme: &Theme) {
        let disk_items: Vec<ListItem> = self.disk_info
            .iter()
            .map(|disk| {
                ListItem::new(vec![
                    Line::from(vec![
                        Span::styled(&disk.name, Style::default().fg(theme.colors.accent).add_modifier(Modifier::BOLD)),
                        Span::raw(" ("),
    ...
    fn render_top_processes(&self, frame: &mut Frame, area: Rect, theme: &Theme) {
        let process_items: Vec<ListItem> = self.top_processes
            .iter()
            .take(area.height.saturating_sub(2) as usize)
            .map(|process| {
                ListItem::new(vec![
                    Line::from(vec![
                        Span::styled(format!("{}", process.pid), Style::default().fg(theme.colors.text_secondary)),
                        Span::raw(" "),
                        Span::styled(&process.name, Style::default().fg(theme.colors.text_primary).add_modifier(Modifier::BOLD)),
                    ]),
                    Line::from(vec![
                        Span::raw("CPU: "),
                        Span::styled(
        ...
        let system_info = vec![
            Line::from(vec![
                ...
                Span::styled(
                    ...
                    Style::default().fg(theme.colors.text_primary).add_modifier(Modifier::BOLD)
                ),
            ]),
            Line::from(vec![
                Span::raw("系统版本: "),
            ...
            Line::from(vec![
                ...
                Span::styled(
                    format!("{:.2}", self.current_status.load_average),
                    Style::default().fg(Self::get_usage_color(self.current_status.load_average * 100.0))
                ),
            ]),
        ];
    ...
    fn render_hardware_info(&self, frame: &mut Frame, area: Rect, theme: &Theme) {
        let hardware_info = vec![
            Line::from(vec![
                Span::raw("CPU型号: "),
                Span::styled(
                    self.cpu_info.as_ref().map_or("检测中...".to_string(), |cpu| cpu.name.clone()),
                    Style::default().fg(theme.colors.text_primary)
                ),
            ]),
            Line::from(vec![
                Span::raw("CPU核心数: "),
                Span::styled(
                    self.cpu_info.as_ref().map_or("未知".to_string(), |cpu| cpu.core_count.to_string()),
                    Style::default().fg(theme.colors.accent)
                ),
                Span::raw(" 核心"),
            ]),
            Line::from(vec![
                Span::raw("CPU频率: "),
                Span::styled(
                    self.cpu_info.as_ref().map_or("未知".to_string(), |cpu| format!("{} MHz", cpu.frequency)),
                    Style::default().fg(theme.colors.text_primary)
                ),
            ]),
            Line::from(vec![
    ...
    fn render_environment_info(&self, frame: &mut Frame, area: Rect, theme: &Theme) {
        let env_info = vec![
            Line::from(vec![
                Span::raw("用户名: "),
                Span::styled(
                    std::env::var("USERNAME").or_else(|_| std::env::var("USER")).unwrap_or_else(|_| "未知".to_string()),
                    Style::default().fg(theme.colors.text_primary)
                ),
            ]),
            Line::from(vec![
            ...
            Line::from(vec![
                ...
                Span::styled(
                    std::env::var("PATH")
                        .map(|p| if p.len() > 50 { format!("{}...", &p[..50]) } else { p })
    ...
    fn render_alert_statistics(&self, frame: &mut Frame, area: Rect, theme: &Theme) {
        ...
        let total_paragraph = Paragraph::new(vec![
            Line::from(vec![
                Span::styled("总警告", Style::default().fg(theme.colors.text_secondary)),
            ]),
            Line::from(vec![
                Span::styled(
        ...
        let critical_paragraph = Paragraph::new(vec![
            Line::from(vec![
                Span::styled("严重", Style::default().fg(Color::Red)),
            ]),
            Line::from(vec![
                Span::styled(
        ...
        let warning_paragraph = Paragraph::new(vec![
            Line::from(vec![
                Span::styled("警告", Style::default().fg(Color::Yellow)),
            ]),
            Line::from(vec![
                Span::styled(
        ...
        let recent_paragraph = Paragraph::new(vec![
            Line::from(vec![
                Span::styled("最近30分钟", Style::default().fg(theme.colors.text_secondary)),
            ]),
            Line::from(vec![
                Span::styled(
    ...
    fn render_threshold_display(&self, frame: &mut Frame, area: Rect, theme: &Theme) {
        ...
        let content = vec![
            Line::from(vec![
                ...
                Span::raw(format!(" {:.0}%/{:.0}% ", thresholds.cpu_warning, thresholds.cpu_critical)),
                Span::styled("内存", Style::default().fg(theme.colors.accent)),
                Span::raw(format!(" {:.0}%/{:.0}% ", thresholds.memory_warning, thresholds.memory_critical)),
                Span::styled("磁盘", Style::default().fg(theme.colors.accent)),
                Span::raw(format!(" {:.0}%/{:.0}%", thresholds.disk_warning, thresholds.disk_critical)),
            ]),
            Line::from(vec![
                Span::styled("按 't' 配置阈值, 按 'c' 清除警告", Style::default().fg(theme.colors.text_secondary)),
            ]),
        ];
            ...
            .map(|alert| {
                ...
                ListItem::new(vec![
                    Line::from(vec![
                        Span::styled(severity_symbol, Style::default().fg(severity_color)),
                        Span::raw(" "),
                        Span::styled(&alert.category, Style::default().fg(theme.colors.accent).add_modifier(Modifier::BOLD)),
                        Span::raw(" - "),
                        Span::styled(format!("{:?}", alert.severity), Style::default().fg(severity_color)),
                    ]),
                    Line::from(vec![
                        Span::raw("时间: "),
                        Span::styled(time_str, Style::default().fg(Color::Gray)),
    ...
    fn render_diagnostics_summary(&self, frame: &mut Frame, area: Rect, theme: &Theme) {
        ...
        let issues_content = if diagnostics.issues.is_empty() {
            vec![Line::from(vec![
                Span::styled("✓ 无严重问题", Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)),
            ])]
        } else {
            diagnostics.issues.iter().take(5).map(|issue| {
                Line::from(vec![
        ...
        let warnings_content = if diagnostics.warnings.is_empty() {
            vec![Line::from(vec![
                Span::styled("✓ 无警告", Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)),
            ])]
        } else {
            diagnostics.warnings.iter().take(5).map(|warning| {
                Line::from(vec![
        ...
        let health_content = vec![
            Line::from(vec![
                ...
                Span::styled(
                    ...
                    Style::default().fg(Self::get_health_color(&diagnostics.overall_health))
                        .add_modifier(Modifier::BOLD)
                ),
            ]),
            Line::from(vec![
            ...
            .map(|rec| {
                ...
                ListItem::new(vec![
                    Line::from(vec![
                        ...
                        Span::raw(" "),
                        Span::styled(&rec.title, Style::default().fg(theme.colors.text_primary).add_modifier(Modifier::BOLD)),
                        Span::raw(" ("),
    ...
    fn render_maintenance_actions(&self, frame: &mut Frame, area: Rect, theme: &Theme) {
        ...
        let action_items: Vec<ListItem> = actions
            ...
            .map(|(name, desc, key)| {
                ListItem::new(vec![
                    Line::from(vec![
                        Span::styled(format!("[{}]", key), Style::default().fg(theme.colors.accent).add_modifier(Modifier::BOLD)),
                        Span::raw(" "),
                        Span::styled(*name, Style::default().fg(theme.colors.text_primary).add_modifier(Modifier::BOLD)),
                    ]),
                    Line::from(vec![
                        Span::raw("    "),
    ...
    fn render_cpu_chart(&self, frame: &mut Frame, area: Rect, theme: &Theme) {
        ...
        chart_lines.push(Line::from(vec![
            Span::raw("CPU使用率 (最近 "),
            Span::styled(self.history.len().to_string(), Style::default().fg(theme.colors.accent)),
            Span::raw(" 个数据点)"),
        ]));
        ...
        if let Some(latest) = self.history.back() {
            chart_lines.push(Line::from(vec![
                ...
                Span::styled(
                    ...
                    Style::default().fg(Self::get_usage_color(latest.cpu_usage)).add_modifier(Modifier::BOLD)
                ),
            ]));
        }
        
        // Simple ASCII chart
        let max_value = self.history.iter()
            .map(|p| p.cpu_usage)
            .fold(0.0, f64::max)
            .max(100.0);
        ...
        if let Some(latest) = self.history.back() {
            chart_lines.push(Line::from(vec![
                ...
                Span::styled(
                    ...
                    Style::default().fg(Self::get_usage_color(latest.memory_usage)).add_modifier(Modifier::BOLD)
                ),
                Span::raw(" ("),
                Span::styled(
        ...
        if let Some(latest) = self.history.back() {
            chart_lines.push(Line::from(vec![
                ...
                Span::styled(
                    ...
                    Style::default().fg(Self::get_usage_color(latest.disk_usage)).add_modifier(Modifier::BOLD)
                ),
            ]));
        }
    ...
    fn render_network_chart(&self, frame: &mut Frame, area: Rect, theme: &Theme) {
        ...
        chart_lines.push(Line::from(vec![
            Span::raw("活动: "),
            Span::styled("监控中...", Style::default().fg(theme.colors.text_secondary)),
        ]));
        ...
        chart_lines.push(Line::from(vec![
            Span::raw("接口: "),
            Span::styled("检测中...", Style::default().fg(theme.colors.text_secondary)),
        ]));
