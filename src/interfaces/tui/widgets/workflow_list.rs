...
use crate::interfaces::tui::action::{Action, InputMode, SortOrder};
...
impl WorkflowListWidget {
        ...
        self.filtered_workflows.sort_by(|&a, &b| {
            ...
            match self.sort_order {
                ...
                SortOrder::Custom(ref custom) => match custom.as_str() {
                    "execution_count_asc" => {
                        workflow_a.execution_count.cmp(&workflow_b.execution_count)
                    }
                    "execution_count_desc" => {
                        workflow_b.execution_count.cmp(&workflow_a.execution_count)
                    }
                    ...
                    "success_rate_asc" => workflow_a
                        .success_rate
                        .partial_cmp(&workflow_b.success_rate)
                        .unwrap_or(std::cmp::Ordering::Equal),
                    "success_rate_desc" => workflow_b
                        .success_rate
                        .partial_cmp(&workflow_a.success_rate)
                        .unwrap_or(std::cmp::Ordering::Equal),
    ...
    fn update_selection(&mut self) {
        if self.selected_index >= self.filtered_workflows.len()
            && !self.filtered_workflows.is_empty()
        {
            self.selected_index = self.filtered_workflows.len() - 1;
        }

        self.list_state
            .select(if self.filtered_workflows.is_empty() {
                None
            } else {
        ...
        self.scroll_state = self
            .scroll_state
            .content_length(self.filtered_workflows.len());
    ...
    fn render_workflow_list(&mut self, frame: &mut Frame, area: Rect, theme: &Theme) {
            ...
            .map(|&index| {
                ...
                let content = vec![
                    ...
                    Line::from(vec![
                        ...
                        Span::raw(" | 执行次数: "),
                        Span::styled(
                            workflow.execution_count.to_string(),
                            Style::default().fg(Color::Cyan),
                        ),
                        Span::raw(" | 节点数: "),
                        Span::styled(
                            workflow.node_count.to_string(),
        ...
        if self.search_mode && area.height > 10 {
            ...
            let help_text = vec![
                Line::from(
                    "过滤语法: status:运行中, tag:数据, version:1.0, nodes:5, nodes>10, nodes<3",
                ),
        ...
        if let Some(workflow) = self.selected_workflow() {
            ...
            let info_lines = vec![
                ...
                Line::from(vec![
                    Span::raw("状态: "),
                    Span::styled(
                        workflow.status_symbol(),
                        Style::default().fg(workflow.status_color()),
                    ),
                    Span::raw(" "),
                    Span::styled(
                        workflow.status_string(),
                        Style::default().fg(workflow.status_color()),
                    ),
                ]),
                Line::from(vec![
            ...
            if let Some(last_exec) = workflow.last_execution {
                ...
                desc_lines.push(Line::from(
                    last_exec.format("%Y-%m-%d %H:%M:%S UTC").to_string(),
                ));
            }
...
impl WorkflowListWidget {
    ...
    async fn handle_key_event(
        ...
        match key.code {
            ...
            KeyCode::Char('f') => Ok(Some(Action::StartInput(InputMode::Filter))),
            KeyCode::Char('c') => {
                self.clear_filter();
            ...
            KeyCode::Char('s') => {
                let next_sort = match self.sort_order {
                    ...
                    SortOrder::Custom(ref custom) => match custom.as_str() {
                        "execution_count_desc" => {
                            SortOrder::Custom("execution_count_asc".to_string())
                        }
