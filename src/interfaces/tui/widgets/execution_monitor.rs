...
use crate::interfaces::tui::Theme;
        ...
        if let Some(execution) = self.selected_execution() {
            let details_lines = vec![
                ...
                Line::from(vec![
                    ...
                    Span::styled(
                        format!("{:.1}%", execution.progress * 100.0),
        ...
        match key.code {
            ...
            KeyCode::Char('a') => {
                ...
                Ok(Some(Action::Custom(
                    "toggle_auto_refresh".to_string(),
                    serde_json::Value::Bool(self.auto_refresh),
                )))
            }
            ...
            KeyCode::Enter => {
                if let Some(execution) = self.selected_execution() {
                    Ok(Some(Action::ShowWorkflowDetails(
            ...
            // Refresh
            KeyCode::F(5) => Ok(Some(Action::Refresh)),
