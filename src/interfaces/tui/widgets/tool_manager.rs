...
use crate::interfaces::tui::widget::{
    SizeConstraints, UpdateFrequency, WidgetCapabilities, WidgetError,
};
use crate::interfaces::tui::{Theme, Widget, WidgetContext, WidgetId};
    ...
    pub fn matches_filter_set(&self, filter_set: &FilterSet) -> bool {
        ...
        if let Some(version_filter) = &filter_set.version_filter {
            if !self
                ...
                .version
                .to_lowercase()
                .contains(&version_filter.to_lowercase())
            {
                return false;
            }
        }
...
impl ToolManagerWidget {
    ...
    pub fn mark_tool_finished(&mut self, tool_name: &str, success: bool, error: Option<String>) {
        ...
        if let Some(tool) = self.tools.iter_mut().find(|t| t.info.name == tool_name) {
            ...
            let successful_executions =
                history.iter().filter(|r| r.success).count() + if success { 1 } else { 0 };
    ...
    pub fn add_execution_record(&mut self, tool_name: &str, record: ToolExecutionRecord) {
        ...
        if let Some(tool) = self.tools.iter_mut().find(|t| t.info.name == tool_name) {
            let durations: Vec<_> = history.iter().filter_map(|r| r.duration).collect();
            ...
            if part.starts_with("status:") {
                ...
                if let Some(range_str) = part.strip_prefix("success:") {
                    if let Some((min_str, max_str)) = range_str.split_once("..") {
                        ...
                        {
                            self.active_filters.success_rate_range =
                                Some((min / 100.0, max / 100.0));
                        }
                    }
                }
            } else {
    ...
    pub fn get_tool_execution_parameters(
        &self,
    ...
    pub fn get_tool_execution_stats(&self, tool_name: &str) -> Option<ToolExecutionStats> {
        if let Some(history) = self.tool_execution_history.get(tool_name) {
            ...
            let durations: Vec<Duration> = history.iter().filter_map(|r| r.duration).collect();
    ...
    pub fn get_recent_execution_errors(
        &self,
        tool_name: &str,
        limit: usize,
        ...
        self.filtered_tools.sort_by(|&a, &b| {
            ...
            match self.sort_order {
                ...
                SortOrder::Custom(ref custom) => match custom.as_str() {
                    ...
                    "success_rate_asc" => tool_a
                        .success_rate
                        .partial_cmp(&tool_b.success_rate)
                        .unwrap_or(std::cmp::Ordering::Equal),
                    "success_rate_desc" => tool_b
                        .success_rate
                        .partial_cmp(&tool_a.success_rate)
                        .unwrap_or(std::cmp::Ordering::Equal),
...
impl ToolManagerWidget {
    ...
    fn render_tool_list(&mut self, frame: &mut Frame, area: Rect, theme: &Theme) {
        ...
        let items: Vec<ListItem> = visible_indices
            ...
            .map(|(display_index, &tool_index)| {
                ...
                let name_spans = if !filter.is_empty()
                    && !filter.starts_with("status:")
                    && !filter.starts_with("category:")
                    && !filter.starts_with("tag:")
                    && !filter.starts_with("source:")
                    && !filter.starts_with("version:")
                {
                    Self::highlight_text_static(
                        &tool.info.name,
                        &filter,
                ...
                if tool.execution_count > 0 {
                    third_line.push(Span::styled(
                        format!(" | 执行次数: {}", tool.execution_count),
                        Style::default().fg(Color::Magenta),
                    ));
                }

                if let Some(rate) = tool.success_rate {
                    let rate_color = if rate >= 0.9 {
                        Color::Green
                    } else if rate >= 0.7 {
                        Color::Yellow
                    } else {
                ...
                if let Some(duration) = tool.average_duration {
                    third_line.push(Span::styled(
                        format!(" | 平均耗时: {}ms", duration.as_millis()),
    ...
    fn render_search_status(&self, frame: &mut Frame, area: Rect, theme: &Theme) {
        ...
        let status = Paragraph::new(status_text).style(if self.search_mode {
    ...
    fn render_pagination_info(&self, frame: &mut Frame, area: Rect, theme: &Theme) {
        ...
        let pagination =
            Paragraph::new(pagination_text).style(Style::default().fg(Color::DarkGray));
    ...
    fn render_tool_parameters(
        ...
        let mut param_lines = vec![Line::from(vec![Span::styled(
            "参数配置:",
            Style::default().add_modifier(Modifier::BOLD),
        )])];
        ...
        if tool.info.parameters_schema.is_null() {
            ...
            {
                for (param_name, param_schema) in properties.iter().take(5) {
                    ...
                    let default_value = param_schema
                        .get("default")
                        .map(|v| format!(" (默认: {})", v))
                        .unwrap_or_default();
                ...
                if properties.len() > 5 {
                    param_lines.push(Line::from(format!(
                        "  ... 还有 {} 个参数",
                        properties.len() - 5
                    )));
                }
    ...
    fn render_tool_info(
        ...
        let info_lines = vec![
            ...
            Line::from(vec![
                ...
                Span::raw(" | 创建时间: "),
                Span::styled(
                    tool.info.created_at.format("%Y-%m-%d").to_string(),
            ...
            Line::from(vec![
                Span::raw("更新时间: "),
                Span::styled(
                    tool.info.updated_at.format("%Y-%m-%d %H:%M").to_string(),
                    Style::default().fg(Color::Gray),
                ),
            ]),
            // Performance metrics
    ...
    fn render_tool_execution_info(
        ...
        if let Some(stats) = self.get_tool_execution_stats(&tool.info.name) {
            lines.push(Line::from(vec![
                ...
                Span::raw(" | 成功: "),
                Span::styled(
                    stats.successful_executions.to_string(),
                    Style::default().fg(Color::Green),
                ),
                Span::raw(" | 失败: "),
                Span::styled(
                    stats.failed_executions.to_string(),
            ...
            lines.push(Line::from(vec![
                ...
                Span::styled(
                    ...
                    Style::default().fg(success_rate_color),
                ),
            ]));
            ...
            if let Some(avg_duration) = stats.average_duration {
                lines.push(Line::from(vec![
                    Span::raw("  平均耗时: "),
                    Span::styled(
                        format!("{}ms", avg_duration.as_millis()),
                        Style::default().fg(Color::Blue),
                    ),
                ]));
            ...
            if let Some(last_exec) = stats.last_execution {
                lines.push(Line::from(vec![
                    Span::raw("  最后执行: "),
                    Span::styled(
                        last_exec.format("%Y-%m-%d %H:%M:%S").to_string(),
                        Style::default().fg(Color::Yellow),
                    ),
                ]));
            }
    ...
    fn render_tool_dependencies(
        ...
        let mut dep_lines = vec![Line::from(vec![Span::styled(
            "依赖关系:",
            Style::default().add_modifier(Modifier::BOLD),
        )])];
        ...
        dep_lines.push(Line::from(vec![Span::styled(
            "版本信息:",
            Style::default().add_modifier(Modifier::BOLD),
        )]));
        ...
        if let Some(history) = self.get_execution_history(&tool.info.name) {
            ...
            dep_lines.push(Line::from(vec![
                Span::raw("  最近执行: "),
                Span::styled(
                    format!("{} 次", recent_executions),
                    Style::default().fg(Color::Yellow),
                ),
            ]));

            if let Some(last_execution) = history.last() {
                let status_text = if last_execution.success {
                    "成功"
                } else {
                    "失败"
                };
                let status_color = if last_execution.success {
                    Color::Green
                } else {
                    Color::Red
                };
        ...
        if self.active_filters.is_empty() && self.filter.is_empty() {
            ...
            if !self.active_filters.tag_filters.is_empty() {
                lines.push(Line::from(vec![
                    Span::raw("  标签: "),
                    Span::styled(
                        self.active_filters.tag_filters.join(", "),
                        Style::default().fg(Color::Yellow),
                    ),
                ]));
            }
            ...
            if !self.active_filters.text_search.is_empty() {
                lines.push(Line::from(vec![
                    Span::raw("  文本: "),
                    Span::styled(
                        &self.active_filters.text_search,
    ...
    async fn handle_key_event(
        ...
        match key.code {
            ...
            KeyCode::Char('r') | KeyCode::F(5) => Ok(Some(Action::RefreshTools)),
            ...
            KeyCode::Char('R') => {
                ...
                if let Some(tool_name) = self.selected_tool_name() {
                    match self.retry_tool_execution(&tool_name) {
                        Ok(()) => Ok(Some(Action::ExecuteTool(tool_name))),
                        Err(error) => {
            ...
            KeyCode::Esc => Ok(Some(Action::Navigate(ViewType::WorkflowList))),
    ...
    async fn handle_search_key_event(
        &mut self,
        key: KeyEvent,
    ) -> std::result::Result<Option<Action>, WidgetError> {
        match key.code {
    ...
    pub async fn show_parameter_input_dialog(
        &mut self,
        tool: &ToolDisplayInfo,
    ) -> Option<serde_json::Value> {
    ...
    fn generate_execution_parameters(
        &self,
    ...
    pub async fn execute_tool_with_monitoring(
        &mut self,
        tool_name: &str,
        parameters: serde_json::Value,
    ...
    pub fn get_tool_performance_metrics(&self, tool_name: &str) -> Option<ToolPerformanceMetrics> {
        if let Some(history) = self.get_execution_history(tool_name) {
            ...
            let recent_success_rate = if !recent_executions.is_empty() {
                recent_executions.iter().filter(|r| r.success).count() as f64
    ...
    fn calculate_executions_per_hour(&self, history: &[ToolExecutionRecord]) -> f64 {
        ...
        let duration_hours = last_execution
            .signed_duration_since(first_execution)
            .num_seconds() as f64
            / 3600.0;
