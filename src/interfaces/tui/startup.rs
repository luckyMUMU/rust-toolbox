...
impl StartupPerformanceManager {
    ...
    pub async fn start_startup(&mut self) -> Result<()> {
        ...
        if config.monitor_startup_time {
            tracing::info!(
                "Starting TUI application startup (target: {}ms)",
    ...
    pub async fn initialize_components(&self) -> Result<StartupMetrics> {
        ...
        if config.enable_parallel_init {
            self.initialize_components_parallel(InitializationPriority::High)
                .await?;
        } else {
            self.initialize_components_sequential(InitializationPriority::High)
                .await?;
        }
        ...
        if !config.enable_fast_boot {
            ...
            if config.enable_parallel_init {
                self.initialize_components_parallel(InitializationPriority::Normal)
                    .await?;
            } else {
                self.initialize_components_sequential(InitializationPriority::Normal)
                    .await?;
            }
    ...
    pub async fn optimize_startup(
        &self,
        metrics: &StartupMetrics,
    ) -> Result<Vec<StartupOptimization>> {
        ...
        if !metrics.target_met {
            optimizations.push(StartupOptimization {
                ...
                description: format!(
                    ...
                    metrics.total_startup_time, config.target_startup_time
                ),
    ...
    async fn end_phase(&self, name: &str) {
        ...
        if let Some(phase) = tracker
            .iter_mut()
            .find(|p| p.name == name && p.end_time.is_none())
        {
            phase.end_time = Some(Instant::now());
            phase.duration = Some(phase.end_time.unwrap() - phase.start_time);

            tracing::debug!(
                "Ended startup phase: {} (duration: {:?})",
                name,
                phase.duration.unwrap()
            );
        }
    }
    ...
    async fn initialize_components_parallel(&self, priority: InitializationPriority) -> Result<()> {
            ...
            let task = tokio::spawn(async move {
                ...
                match result {
                    ...
                    Err(_) => {
                        ...
                        status.insert(
                            name.clone(),
                            InitializationStatus::Failed(error_msg.clone()),
                        );
        ...
        if failed_count > 0 {
            tracing::warn!(
                "{} components failed to initialize in parallel batch",
                failed_count
            );
        }
    ...
    async fn initialize_components_sequential(
        &self,
    ...
    async fn initialize_single_component(
        &self,
        name: &str,
        component: Arc<dyn ComponentInitializer>,
    ) -> Result<()> {
        let start_time = Instant::now();
        ...
        match result {
            ...
            Ok(Err(e)) => {
                ...
                status.insert(
                    name.to_string(),
                    InitializationStatus::Failed(e.to_string()),
                );
                ...
                for (name, component) in background_components {
                    ...
                    match component.initialize().await {
                        Ok(()) => {
                            ...
                            tracing::debug!(
                                "Background initialized component '{}' in {:?}",
                                name,
                                duration
                            );
                        }
                        Err(e) => {
                            let mut status = component_status.write().await;
                            status
                                .insert(name.clone(), InitializationStatus::Failed(e.to_string()));

                            tracing::error!(
                                "Failed to background initialize component '{}': {}",
                                name,
                                e
                            );
                        }
    ...
    async fn generate_startup_metrics(&self) -> StartupMetrics {
        ...
        let startup_phases: HashMap<String, Duration> = phases
            ...
            .filter_map(|phase| {
                phase
                    .duration
                    .map(|duration| (phase.name.clone(), duration))
            })
            .collect();
...
mod tests {
    ...
    async fn test_component_initialization() {
        ...
        manager
            .register_component(critical_component)
            .await
            .unwrap();
    ...
    async fn test_startup_optimization() {
        ...
        let has_slow_component_optimization = optimizations.iter().any(|opt| {
            matches!(
                opt.optimization_type,
                StartupOptimizationType::OptimizeSlowComponent
            )
        });
