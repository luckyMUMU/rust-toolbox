...
use crate::workflow::{WorkflowExecution, NodeExecutionState};
...
impl OutputFormatter for TableFormatter {
    fn format_workflow_status(&self, execution: &WorkflowExecution) -> String {
        ...
        if !execution.node_states.is_empty() {
            ...
            for (node_id, node_state) in &execution.node_states {
                let started = node_state.started_at
                    .map(|t| t.format("%Y-%m-%d %H:%M:%S").to_string())
                    .unwrap_or_else(|| "-".to_string());
                let completed = node_state.completed_at
                    .map(|t| t.format("%Y-%m-%d %H:%M:%S").to_string())
                    .unwrap_or_else(|| "-".to_string());
    ...
    fn format_execution_result(&self, result: &ExecutionResult) -> String {
        ...
        if let Some(result_value) = &result.result {
            output.push_str(&format!("Result: {}\n", serde_json::to_string_pretty(result_value).unwrap_or_else(|_| "Invalid JSON".to_string())));
        }
...
impl OutputFormatter for TextFormatter {
    fn format_workflow_status(&self, execution: &WorkflowExecution) -> String {
        format!("{} {} {:?}", execution.id, execution.workflow_name, execution.status)
    }
    ...
    fn format_execution_result(&self, result: &ExecutionResult) -> String {
        format!("{} {} {:?}", result.workflow_id, result.workflow_name, result.status)
    }
...
fn calculate_progress(node_states: &HashMap<String, NodeExecutionState>) -> String {
    ...
    let completed = node_states.values()
...
mod tests {
    ...
    fn create_test_execution() -> WorkflowExecution {
        ...
        node_states.insert("node1".to_string(), NodeExecutionState {
        ...
        node_states.insert("node2".to_string(), NodeExecutionState {
