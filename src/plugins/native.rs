...
type PluginExecuteToolFn = unsafe extern "C" fn(*mut c_void, *const c_char, *const c_char, *const c_char) -> *const c_char;
...
impl NativePlugin {
    ...
    fn load_library(&mut self) -> Result<()> {
        ...
        debug!("Successfully loaded native library: {:?}", self.library_path);
        Ok(())
    }
    ...
    fn verify_symbols(&self, library: &Library) -> Result<()> {
        ...
        for symbol_name in &required_symbols {
            unsafe {
                let _: Symbol<unsafe extern "C" fn()> = library.get(symbol_name.as_bytes()).map_err(|e| {
                    WorkflowError::plugin(format!(
        ...
        debug!("All required symbols found in library: {:?}", self.library_path);
        Ok(())
    }
    ...
    fn initialize_native_plugin(&mut self, config: &PluginConfig) -> Result<()> {
        let library = self.library.as_ref().ok_or_else(|| {
            WorkflowError::plugin("Library not loaded".to_string())
        })?;
    ...
    fn load_tools(&mut self) -> Result<()> {
        ...
        let plugin_handle = self.plugin_handle.ok_or_else(|| {
            WorkflowError::plugin("Plugin not initialized".to_string())
        })?;
            ...
            let parameters_schema: Value = if tool_descriptor.parameters_schema.is_null() {
                ...
                let schema_str = unsafe {
                    CStr::from_ptr(tool_descriptor.parameters_schema)
                        .to_str()
                        .map_err(|e| WorkflowError::plugin(format!("Invalid parameters schema: {}", e)))?
                };
            ...
            let return_schema: Value = if tool_descriptor.return_schema.is_null() {
                ...
                let schema_str = unsafe {
                    CStr::from_ptr(tool_descriptor.return_schema)
                        .to_str()
                        .map_err(|e| WorkflowError::plugin(format!("Invalid return schema: {}", e)))?
                };
                serde_json::from_str(schema_str).unwrap_or(Value::Null)
            };
    ...
    fn validate_security_policy(&self, policy: &SecurityPolicy) -> Result<()> {
        if !policy.sandbox_enabled {
            warn!("Native plugin '{}' has sandbox disabled - this may be unsafe", self.info.name);
        }
        
        if policy.allow_network_access {
            warn!("Native plugin '{}' has network access enabled", self.info.name);
        }
...
impl Plugin for NativePlugin {
    ...
    fn initialize(&mut self, config: PluginConfig) -> Result<()> {
        ...
        info!("Native plugin '{}' initialized successfully", self.info.name);
        Ok(())
    }
    ...
    fn shutdown(&mut self) -> Result<()> {
        if matches!(self.status, PluginStatus::Shutdown | PluginStatus::Uninitialized) {
            return Ok(());
        }
...
impl ToolExecutor for NativeToolExecutor {
    async fn execute(&self, params: Value, context: ExecutionContext) -> Result<Value> {
        ...
        let library = unsafe {
            Library::new(&self.library_path).map_err(|e| {
                WorkflowError::tool(format!(
                    "Failed to load library for tool execution: {}",
                    e
                ))
            })?
        };
        ...
        let params_json = serde_json::to_string(&params).map_err(|e| {
            WorkflowError::tool(format!("Failed to serialize parameters: {}", e))
        })?;
        
        let context_json = serde_json::to_string(&context).map_err(|e| {
            WorkflowError::tool(format!("Failed to serialize context: {}", e))
        })?;
        ...
        let result: Value = serde_json::from_str(result_str).map_err(|e| {
            WorkflowError::tool(format!("Failed to deserialize result: {}", e))
        })?;
...
impl NativePluginBuilder {
    ...
    pub fn build(self) -> Result<NativePlugin> {
        let info = self.info.ok_or_else(|| {
            WorkflowError::ValidationError("Plugin info is required".to_string())
        })?;
