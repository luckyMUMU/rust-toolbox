...
impl FileManagementError {
    ...
    pub fn is_recoverable(&self) -> bool {
        match self {
            ...
            Self::Conflict { context, .. } => true, // Can be resolved with different strategy
            Self::InvalidPath { context, .. } => context.retryable,
            Self::TextProcessing { context, .. } => true, // Might work with different input
            Self::Classification { context, .. } => true, // Might work with different rules
            Self::PatternMatching { context, .. } => true, // Might work with different patterns
            Self::HumanDecisionTimeout { context, .. } => true, // Can retry with longer timeout
            Self::HumanDecisionCancelled { context, .. } => context.retryable,
            Self::ExperimentalModeViolation { context, .. } => false,
            Self::BatchProcessing { context, .. } => true, // Can retry failed items
            Self::Configuration { context, .. } => context.retryable,
            Self::Validation { context, .. } => context.retryable,
            Self::Io { context, .. } => true, // Might be temporary
    ...
    pub fn severity(&self) -> ErrorSeverity {
        match self {
            ...
            Self::ExperimentalModeViolation { context, .. } => ErrorSeverity::Low,
            Self::BatchProcessing { context, .. } => context.severity,
            Self::Configuration { context, .. } => ErrorSeverity::High,
            Self::Validation { context, .. } => ErrorSeverity::Medium,
            Self::Io { context, .. } => context.severity,
            Self::Json { context, .. } => context.severity,
            Self::Performance { context, .. } => ErrorSeverity::Medium,
            Self::ResourceExhaustion { context, .. } => ErrorSeverity::High,
            Self::Timeout { context, .. } => ErrorSeverity::Medium,
            Self::Concurrency { context, .. } => ErrorSeverity::Medium,
            Self::Recovery { context, .. } => ErrorSeverity::High,
