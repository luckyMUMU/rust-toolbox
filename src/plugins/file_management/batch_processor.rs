...
impl BatchProcessor {
    ...
    pub async fn process_batch(
        ...
        info!("Starting batch processing: {} items with tool '{}'", items.len(), tool_name);
        ...
        let batch_result = self.create_batch_result(
            batch_id,
            tool_name.to_string(),
            final_progress,
            all_results,
    ...
    async fn get_tool(&self, tool_name: &str) -> FileManagementResult<Arc<dyn ToolNode>> {
        ...
        registry.get_tool(tool_name)
            .ok_or_else(|| FileManagementError::validation(&format!("Tool '{}' not found", tool_name)))
    }
    ...
    async fn process_single_item(
        ...
        // Retry loop
        loop {
            let result = Self::execute_item_with_timeout(
                &tool,
                &item,
                &context,
                config.item_timeout,
            ...
            match result {
                Ok(value) => {
                    ...
                    {
                        let mut progress_guard = progress.write().await;
                        progress_guard.processing_items = progress_guard.processing_items.saturating_sub(1);
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                debug!(
                    ...
                    progress_guard.completed_items + progress_guard.failed_items + progress_guard.skipped_items,
    ...
    fn create_performance_metrics(&self, item_results: &[BatchItemResult], total_duration: Duration) -> BatchPerformanceMetrics {
        ...
        let concurrency_utilization = if actual_parallel_time.as_secs() > 0 {
            (theoretical_sequential_time.as_secs_f64() / actual_parallel_time.as_secs_f64()) / self.config.max_concurrency as f64
        } else {
            0.0
        }.min(1.0);
...
mod tests {
    ...
    async fn test_simple_batch_processing() {
        ...
        let result = processor.process_batch("test-tool", items, context).await.unwrap();
    ...
    async fn test_batch_processing_with_failures() {
        ...
        // Process batch
        let context = ExecutionContext::new();
        let result = processor.process_batch("failing-tool", items, context).await.unwrap();
    ...
    async fn test_batch_processing_concurrency() {
        ...
        // Process batch
        let context = ExecutionContext::new();
        let result = processor.process_batch("slow-tool", items, context).await.unwrap();
    ...
    async fn test_batch_item_priority_ordering() {
        ...
        // Process batch
        let context = ExecutionContext::new();
        let result = processor.process_batch("priority-tool", items, context).await.unwrap();
        ...
        // (high -> medium -> low)
        let call_counts: Vec<i64> = result.item_results.iter()
            .map(|r| r.result.as_ref().unwrap()["call_count"].as_i64().unwrap())
            .collect();
        
        // Find the call count for each priority item
        let high_priority_call = result.item_results.iter()
            .find(|r| r.id == "high-priority")
            .unwrap()
            .result.as_ref().unwrap()["call_count"].as_i64().unwrap();
        
        let medium_priority_call = result.item_results.iter()
            .find(|r| r.id == "medium-priority")
            .unwrap()
            .result.as_ref().unwrap()["call_count"].as_i64().unwrap();
        
        let low_priority_call = result.item_results.iter()
