...
use super::batch_processor::{BatchProgress, BatchItemResult, BatchItemStatus, BatchPerformanceMetrics};
...
impl ProgressTracker {
    ...
    pub async fn start_batch(&self, batch_id: String, tool_name: String, total_items: usize) {
        ...
        // Store batch state
        self.active_batches.write().await.insert(batch_id.clone(), batch_state);
    ...
    pub async fn start_item(&self, batch_id: &str, item_id: String) {
        if let Some(batch_state) = self.active_batches.write().await.get_mut(batch_id) {
            ...
            batch_state.progress.pending_items = batch_state.progress.pending_items.saturating_sub(1);
    ...
    pub async fn complete_item(&self, batch_id: &str, item_result: BatchItemResult) {
        if let Some(batch_state) = self.active_batches.write().await.get_mut(batch_id) {
            ...
            batch_state.progress.processing_items = batch_state.progress.processing_items.saturating_sub(1);
            ...
            if self.config.track_item_events {
                let result_size = item_result.result.as_ref()
            ...
            self.performance_monitor.record_item_completion(&item_result).await;
        }
    }
    
    /// Record an error during processing
    pub async fn record_error(&self, batch_id: &str, item_id: Option<String>, error: &str, error_type: &str) {
        ...
        *stats.common_error_types.entry(error_type.to_string()).or_insert(0) += 1;
    }
    ...
    pub async fn complete_batch(&self, batch_id: &str) {
        if let Some(batch_state) = self.active_batches.write().await.remove(batch_id) {
            ...
            self.update_aggregated_stats(&batch_state, total_duration).await;
            
            // Record performance trend
            self.performance_monitor.record_batch_completion(&batch_state, total_duration).await;
        }
    }
    ...
    async fn update_aggregated_stats(&self, batch_state: &BatchProgressState, total_duration: Duration) {
        let mut stats = self.aggregated_stats.write().await;
        ...
        let total_batch_time = stats.average_batch_duration * (stats.total_batches - 1) as u32 + total_duration;
        ...
        {
            let tool_stats = stats.tool_usage_stats.entry(batch_state.tool_name.clone()).or_insert_with(|| ToolUsageStats {
                usage_count: 0,
                items_processed: 0,
                success_rate: 0.0,
            ...
            let total_tool_time = tool_stats.average_duration * (tool_stats.usage_count - 1) as u32 + total_duration;
        ...
        let total_throughput = stats.average_throughput * (stats.total_batches - 1) as f64 + current_throughput;
...
impl PerformanceMonitor {
    ...
    pub async fn record_batch_completion(&self, batch_state: &BatchProgressState, total_duration: Duration) {
        if self.config.calculate_metrics {
            ...
            let success_rate = if batch_state.progress.total_items > 0 {
                batch_state.progress.completed_items as f64 / batch_state.progress.total_items as f64
            } else {
                0.0
            };
    ...
    pub async fn get_performance_trends(&self, limit: Option<usize>) -> Vec<PerformanceTrend> {
        ...
        for (timestamp, throughput) in throughput_history[start_idx..].iter() {
            ...
            let success_rate = success_rate_history.iter()
...
mod tests {
    ...
    async fn test_event_subscription() {
        ...
        tracker.start_batch(batch_id.clone(), tool_name.clone(), 1).await;
        
        // Receive the event
        let event = receiver.recv().await.unwrap();
        match event {
            ProgressEvent::BatchStarted { batch_id: received_id, tool_name: received_tool, total_items, .. } => {
    ...
    async fn test_error_tracking() {
        ...
        // Record some errors
        tracker.record_error(&batch_id, Some("item-1".to_string()), "File not found", "not_found").await;
        tracker.record_error(&batch_id, Some("item-2".to_string()), "Permission denied", "permission").await;
        tracker.record_error(&batch_id, Some("item-3".to_string()), "File not found", "not_found").await;
    ...
    async fn test_performance_monitoring() {
        ...
        monitor.record_batch_completion(&batch_state, Duration::from_millis(300)).await;
    ...
    async fn test_progress_reporting_task() {
        ...
        tracker.start_batch(batch_id.clone(), "test-tool".to_string(), 1).await;
        
        // Skip the BatchStarted event
        let _ = receiver.recv().await.unwrap();
        
        // Wait for a progress update
        let event = tokio::time::timeout(Duration::from_millis(500), receiver.recv()).await.unwrap().unwrap();
        
        match event {
            ProgressEvent::ProgressUpdate { batch_id: received_id, .. } => {
                assert_eq!(received_id, batch_id);
            }
    ...
    async fn test_event_history() {
        ...
        for i in 0..10 {
            ...
            tracker.start_batch(batch_id, "test-tool".to_string(), 1).await;
        }
