...
use super::audit::{AuditLogger, AuditEventType, LogLevel, AuditQueryCriteria};
...
async fn test_audit_event_logging() {
    ...
    let result = audit_logger.log_audit_event(workflow_started_event.clone()).await;
    assert!(result.is_ok(), "Should successfully log audit event: {:?}", result);
    ...
    assert!(result.is_ok(), "Should successfully log node event: {:?}", result);
    ...
    assert!(result.is_ok(), "Should successfully log execution entry: {:?}", result);
    ...
    assert!(events.iter().any(|e| e.event_type == AuditEventType::WorkflowStarted), 
           "Should find workflow started event");
    assert!(events.iter().any(|e| e.event_type == AuditEventType::NodeStarted), 
...
async fn test_execution_log_querying() {
    ...
    let error_logs = audit_logger.query_execution_logs(
        workflow_id,
        Some(&execution_id),
        Some(LogLevel::Error),
        None,
    ...
    assert_eq!(error_logs[0].level, LogLevel::Error, "Should be error level log");
}
...
async fn test_audit_report_generation() {
    ...
    let report = audit_logger.generate_audit_report(start_time, end_time).await.unwrap();
    
    assert!(report.total_events >= 4, "Report should contain at least 4 events");
    assert!(report.workflows_affected.contains(&workflow_id), "Report should include the test workflow");
    assert!(!report.report_id.is_empty(), "Report should have a valid ID");
    assert!(report.events_by_type.contains_key(&AuditEventType::WorkflowStarted), 
...
async fn test_audit_cleanup() {
    ...
    let events_before = audit_logger.query_audit_events(criteria.clone()).await.unwrap();
    assert!(!events_before.is_empty(), "Should find the logged event before cleanup");
    ...
    assert!(cleanup_result.is_ok(), "Cleanup should complete without error: {:?}", cleanup_result);
}