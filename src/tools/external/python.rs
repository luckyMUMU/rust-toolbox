...
impl PythonExecutor {
    ...
    /// 使用指定 Python 路径创建执行器
    pub fn with_python_path(config: ExternalExecutorConfig, python_path: String) -> Self {
        Self {
            config,
            python_path,
        }
...
impl ExternalExecutor for PythonExecutor {
    async fn execute(
        &self,
        script: &str,
        args: &[String],
        ...
        let mut child = cmd.spawn().map_err(|e| {
            error!("启动 Python 进程失败: {}", e);
            WorkflowError::execution(format!("启动 Python 进程失败: {}", e))
        })?;
        ...
        let result = tokio::time::timeout(timeout_duration, async {
            ...
            let status = child
                .wait()
                .await
                .map_err(|e| WorkflowError::execution(format!("等待进程结束失败: {}", e)))?;
...
mod tests {
    ...
    async fn test_python_executor_creation() {
        let executor = PythonExecutor::new(ExternalExecutorConfig::default());
        assert_eq!(executor.name(), "python");
    }
