        ...
        let parameter_schema = if info.parameters_schema != Value::Null {
            Some(
                JSONSchema::options()
                    ...
                    .map_err(|e| {
                        WorkflowError::ValidationError(format!("Invalid parameter schema: {}", e))
                    })?,
            )
        } else {
            None
...
impl BasicToolBuilder {
    ...
    pub fn dependencies<I: IntoIterator<Item = S>, S: Into<String>>(
        mut self,
        dependencies: I,
    ) -> Self {
    ...
    pub fn version_requirement<S1: Into<String>, S2: Into<String>>(
        mut self,
        tool_name: S1,
        requirement: S2,
    ) -> Self {
        self.version_requirements
            .insert(tool_name.into(), requirement.into());
        self
    }
    ...
    where
        ...
        S2: Into<String>,
    {
        self.version_requirements = requirements
            .into_iter()
    ...
    pub fn parameter_templates<I: IntoIterator<Item = ParameterTemplate>>(
        mut self,
        templates: I,
    ) -> Self {
    ...
    pub fn build(self) -> Result<BasicTool> {
        let name = self
            .name
            .ok_or_else(|| WorkflowError::ValidationError("Tool name is required".to_string()))?;
        let version = self.version.ok_or_else(|| {
            WorkflowError::ValidationError("Tool version is required".to_string())
        })?;
        let description = self
            .description
            .unwrap_or_else(|| format!("Tool: {}", name));
        let executor = self.executor.ok_or_else(|| {
            WorkflowError::ValidationError("Tool executor is required".to_string())
        })?;
        ...
        BasicTool::new(info, executor, self.plugin_info).map(|mut tool| {
