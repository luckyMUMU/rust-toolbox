...
impl TemplateContext {
    ...
    fn add_builtin_functions(&mut self) {
        ...
        self.functions.insert("env".to_string(), Box::new(EnvFunction));
        
        // default(value, default_value) - use default if value is null/empty
        self.functions.insert("default".to_string(), Box::new(DefaultFunction));
        
        // upper(text) - convert to uppercase
        self.functions.insert("upper".to_string(), Box::new(UpperFunction));
        
        // lower(text) - convert to lowercase
        self.functions.insert("lower".to_string(), Box::new(LowerFunction));
        
        // join(array, separator) - join array elements
        self.functions.insert("join".to_string(), Box::new(JoinFunction));
        
        // split(text, separator) - split text into array
        self.functions.insert("split".to_string(), Box::new(SplitFunction));
        
        // length(value) - get length of string or array
        self.functions.insert("length".to_string(), Box::new(LengthFunction));
        
        // format(template, ...args) - format string with arguments
        self.functions.insert("format".to_string(), Box::new(FormatFunction));
    }
}
...
impl TemplateEngine {
    ...
    pub fn new() -> Result<Self> {
        let variable_pattern = Regex::new(r"\$\{([^}]+)\}")
            .map_err(|e| WorkflowError::ValidationError(format!("Invalid variable regex: {}", e)))?;
        
        let function_pattern = Regex::new(r"([a-zA-Z_][a-zA-Z0-9_]*)\s*\(\s*([^)]*)\s*\)")
            .map_err(|e| WorkflowError::ValidationError(format!("Invalid function regex: {}", e)))?;
    ...
    pub fn expand(&self, value: &Value, context: &TemplateContext) -> Result<Value> {
        match value {
            ...
            Value::Array(arr) => {
                let expanded: Result<Vec<Value>> = arr.iter()
                    .map(|v| self.expand(v, context))
                    .collect();
            ...
            Value::Object(obj) => {
                ...
                for (key, val) in obj {
                    let expanded_key = if let Value::String(expanded_key) = self.expand_string(key, context)? {
                        expanded_key
                    } else {
                        key.clone()
                    };
    ...
    pub fn expand_string(&self, template: &str, context: &TemplateContext) -> Result<Value> {
        ...
        if iterations >= MAX_ITERATIONS {
            warn!("Template expansion reached maximum iterations for: {}", template);
        }
    ...
    fn evaluate_expression(&self, expr: &str, context: &TemplateContext) -> Result<Value> {
        ...
        Err(WorkflowError::ValidationError(
            format!("Unknown variable or expression: {}", expr)
        ))
    }
    ...
    fn evaluate_nested_property(&self, expr: &str, context: &TemplateContext) -> Result<Value> {
        ...
        if parts.is_empty() {
            return Err(WorkflowError::ValidationError("Empty property path".to_string()));
        }
        
        let mut current = context.get_variable(parts[0])
            ...
            match &current {
                Value::Object(obj) => {
                    current = obj.get(*part)
                        .ok_or_else(|| WorkflowError::ValidationError(
                            format!("Property '{}' not found", part)
                        ))?
                        .clone();
                }
                Value::Array(arr) => {
                    let index = part.parse::<usize>()
                        .map_err(|_| WorkflowError::ValidationError(
                ...
                _ => {
                    return Err(WorkflowError::ValidationError(
                        format!("Cannot access property '{}' on non-object/array", part)
                    ));
                }
            }
        }
...
impl TemplateFn for EnvFunction {
    fn call(&self, args: Vec<Value>) -> Result<Value> {
        ...
        let var_name = args[0].as_str()
            .ok_or_else(|| WorkflowError::ValidationError(
                "env() function expects string argument".to_string()
            ))?;
    ...
    pub fn expand(&self, context: &mut TemplateContext, engine: &TemplateEngine) -> Result<Value> {
        ...
        for var in &self.required_variables {
            if context.get_variable(var).is_none() {
                return Err(WorkflowError::ValidationError(
                    format!("Required template variable '{}' not provided", var)
                ));
            }
        }
...
mod tests {
    ...
    fn test_template_functions() {
        ...
        let result = engine.expand_string("${default(missing, 'default_value')}", &context).unwrap();
    ...
    fn test_nested_properties() {
        ...
        let result = engine.expand_string("${config.database.host}:${config.database.port}", &context).unwrap();
