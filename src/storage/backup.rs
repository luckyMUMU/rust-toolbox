...
use crate::storage::{RetentionPolicy, StorageBackend};
...
impl BackupManager {
    ...
    pub async fn create_incremental_backup(
        ...
        let workflow_states = self
            .collect_changed_workflow_states(&previous_backup)
            .await?;

        // Collect new execution records since previous backup
        let execution_records = if self.config.include_execution_history {
            self.collect_execution_records_since(&previous_backup.metadata.created_at)
                .await?
        } else {
            Vec::new()
        };
    ...
    pub async fn restore_from_backup(&self, backup_id: &str) -> Result<RestoreResult> {
        ...
        for (workflow_id, workflow_state) in full_backup_data.workflow_states {
                ...
                .await
            {
                ...
                Err(e) => restore_result
                    .errors
                    .push(format!("Failed to restore workflow {}: {}", workflow_id, e)),
            }
        }
        ...
        for execution_record in full_backup_data.execution_records {
            match self.restore_execution_record(execution_record).await {
                ...
                Err(e) => restore_result
                    .errors
                    .push(format!("Failed to restore execution record: {}", e)),
            }
        }
        ...
        for (key, value) in full_backup_data.system_data {
            match self.restore_system_data(&key, value).await {
                ...
                Err(e) => restore_result
                    .errors
                    .push(format!("Failed to restore system data {}: {}", key, e)),
            }
        }
        ...
        match self.load_backup(backup_id).await {
            Ok(backup_data) => {
                ...
                if let Err(e) = self.verify_data_integrity(&backup_data).await {
                    ...
                    verification
                        .errors
                        .push(format!("Data integrity check failed: {}", e));
                }
            }
            Err(e) => {
                verification.is_valid = false;
                verification
                    .errors
                    .push(format!("Failed to load backup: {}", e));
            }
        }
...
impl BackupManager {
    ...
    async fn collect_changed_workflow_states(
        &self,
        previous_backup: &BackupData,
    ) -> Result<HashMap<WorkflowId, WorkflowState>> {
    ...
    async fn collect_execution_records_since(
        &self,
        since: &DateTime<Utc>,
    ...
    async fn collect_changed_system_data(
        &self,
        _previous_backup: &BackupData,
        ...
        while let Some(backup_id) = current_backup_id {
            ...
            for (workflow_id, workflow_state) in previous_backup.workflow_states {
                full_data
                    .workflow_states
                    .entry(workflow_id)
                    .or_insert(workflow_state);
            }

            // Merge execution records
            full_data
                .execution_records
                .extend(previous_backup.execution_records);
    ...
    async fn restore_workflow_state(
        &self,
    ...
    async fn restore_execution_record(&self, execution_record: ExecutionRecord) -> Result<()> {
        let key = format!(
            "execution:history:{}:{}",
            execution_record.workflow_id, execution_record.execution_id
        );
    ...
    async fn verify_data_integrity(&self, backup_data: &BackupData) -> Result<()> {
        ...
        for (workflow_id, workflow_state) in &backup_data.workflow_states {
            serde_json::to_vec(workflow_state).map_err(|e| {
                crate::error::WorkflowError::BackupError(format!(
        ...
        for execution_record in &backup_data.execution_records {
            serde_json::to_vec(execution_record).map_err(|e| {
                crate::error::WorkflowError::BackupError(format!(
