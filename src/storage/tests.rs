...
mod tests {
    ...
    use crate::storage::{CacheBackend, StorageBackend, SimpleMemoryCache, FileStorage, LocalMemoryCache, CacheConfig, StateManager};
    ...
    async fn test_file_storage() {
        ...
        assert_eq!(storage.load("key1").await.unwrap(), Some(b"value1".to_vec()));
        assert!(storage.exists("key1").await.unwrap());
        ...
        let batch_keys = vec!["batch1".to_string(), "batch2".to_string(), "nonexistent".to_string()];
    ...
    async fn test_state_manager() {
        ...
        use crate::workflow::{WorkflowState, WorkflowExecution, ExecutionRecord};
        ...
        assert!(!state_manager.workflow_state_exists(workflow_id).await.unwrap());
        
        state_manager.save_workflow_state(workflow_id, workflow_state.clone()).await.unwrap();
        assert!(state_manager.workflow_state_exists(workflow_id).await.unwrap());
        
        let loaded_state = state_manager.load_workflow_state(workflow_id).await.unwrap();
        assert!(loaded_state.is_some());
        assert_eq!(loaded_state.unwrap().execution.workflow_name, "test_workflow");
        ...
        state_manager.save_execution_record(execution_record.clone()).await.unwrap();
        
        let history = state_manager.get_execution_history(workflow_id).await.unwrap();
        assert_eq!(history.len(), 1);
        assert_eq!(history[0].execution_id, "exec1");
        
        let record = state_manager.get_execution_record(workflow_id, "exec1").await.unwrap();
        ...
        // Test cleanup
        state_manager.delete_workflow_state(workflow_id).await.unwrap();
        assert!(!state_manager.workflow_state_exists(workflow_id).await.unwrap());
        
        state_manager.delete_execution_record(workflow_id, "exec1").await.unwrap();
        let history_after_delete = state_manager.get_execution_history(workflow_id).await.unwrap();
