...
impl MetricsCollector {
    ...
    pub async fn record_execution(&self, component: &str, duration: Duration, memory_usage: MemoryUsage) {
        let config = self.config.read().await;
        if !config.enabled {
            return;
        }
        
        let mut metrics = self.execution_metrics.entry(component.to_string()).or_insert_with(ExecutionMetrics::default);
        ...
        self.record_time_series(&format!("{}_duration", component), duration.as_millis() as f64).await;
        self.record_time_series(&format!("{}_memory", component), memory_usage.allocated as f64).await;
    }
    
    /// Record custom metric
    pub async fn record_custom_metric(&self, name: &str, value: f64, labels: Option<HashMap<String, String>>) {
        let config = self.config.read().await;
        if !config.enabled {
            return;
        }
        ...
        let mut metric = self.custom_metrics.entry(name.to_string()).or_insert_with(|| {
            CustomMetric {
                name: name.to_string(),
    ...
    pub async fn get_execution_metrics(&self, component: &str) -> Option<ExecutionMetrics> {
        self.execution_metrics.get(component).map(|entry| entry.clone())
    }
    
    /// Get all execution metrics
    pub async fn get_all_execution_metrics(&self) -> HashMap<String, ExecutionMetrics> {
        self.execution_metrics.iter().map(|entry| (entry.key().clone(), entry.value().clone())).collect()
    }
    ...
    pub async fn get_all_custom_metrics(&self) -> HashMap<String, CustomMetric> {
        self.custom_metrics.iter().map(|entry| (entry.key().clone(), entry.value().clone())).collect()
    }
    ...
    pub async fn export_prometheus(&self) -> String {
        ...
        for entry in self.execution_metrics.iter() {
            ...
            output.push_str(&format!(
                ...
                component, metrics.total_duration.as_secs_f64()
            ));
            output.push_str(&format!(
    ...
    async fn record_time_series(&self, name: &str, value: f64) {
        let config = self.config.read().await;
        let mut series = self.time_series.entry(name.to_string()).or_insert_with(TimeSeries::new);
...
mod tests {
    ...
    async fn test_execution_metrics_recording() {
        ...
        collector.record_execution("test_component", Duration::from_millis(100), memory_usage).await;
        
        let metrics = collector.get_execution_metrics("test_component").await.unwrap();
    ...
    async fn test_prometheus_export() {
        ...
        collector.record_execution("test", Duration::from_millis(100), memory_usage).await;
