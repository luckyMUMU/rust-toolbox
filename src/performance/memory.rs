...
impl MemoryManager {
    ...
    pub fn new(config: MemoryConfig) -> Self {
        ...
        let cleanup_scheduler = Arc::new(CleanupScheduler::new(
            config.clone(),
            usage_history.clone(),
        ));
    ...
    pub async fn record_usage(&self, component: &str, usage: MemoryUsage) {
        let mut history = self.usage_history.entry(component.to_string()).or_insert_with(Vec::new);
    ...
    pub async fn optimize(&self) -> crate::Result<Vec<MemoryOptimization>> {
        ...
        optimizations.sort_by(|a, b| {
            match (&a.priority, &b.priority) {
                ...
                (OptimizationPriority::High, OptimizationPriority::Low | OptimizationPriority::Medium) => std::cmp::Ordering::Less,
                (OptimizationPriority::Low | OptimizationPriority::Medium, OptimizationPriority::High) => std::cmp::Ordering::Greater,
    ...
    async fn analyze_component_usage(&self, component: &str, history: &[MemoryUsage]) -> Option<MemoryOptimization> {
        if history.is_empty() {
            return None;
        }
        ...
        if avg_usage > config.max_tool_memory / 2 {
            return Some(MemoryOptimization {
                ...
                description: format!("Component '{}' is using high average memory: {} bytes", component, avg_usage),
        ...
        if history.len() >= 10 {
            let recent_avg = history[history.len()-5..].iter().map(|u| u.final_usage).sum::<usize>() / 5;
            let older_avg = history[history.len()-10..history.len()-5].iter().map(|u| u.final_usage).sum::<usize>() / 5;
            
            if recent_avg > older_avg * 2 {
                return Some(MemoryOptimization {
                    optimization_type: MemoryOptimizationType::IncreaseCleanupFrequency,
                    description: format!("Potential memory leak detected in component '{}'", component),
    ...
    async fn generate_pressure_optimizations(&self) -> Vec<MemoryOptimization> {
        vec![
            MemoryOptimization {
                optimization_type: MemoryOptimizationType::IncreaseCleanupFrequency,
                description: "System under memory pressure - increase cleanup frequency".to_string(),
    ...
    async fn get_component_usage_summary(&self) -> HashMap<String, ComponentUsageSummary> {
        ...
        for entry in self.usage_history.iter() {
            ...
            if !history.is_empty() {
                ...
                summary.insert(component.clone(), ComponentUsageSummary {
...
impl MemoryPressureDetector {
    ...
    pub async fn check_pressure(&self, usage: &MemoryUsage) {
        ...
        if new_level != *current_level {
            tracing::info!("Memory pressure level changed: {:?} -> {:?}", *current_level, new_level);
            *current_level = new_level;
        }
    }
    
    pub async fn is_under_pressure(&self) -> bool {
        matches!(*self.pressure_level.read().await, PressureLevel::High | PressureLevel::Critical)
    }
