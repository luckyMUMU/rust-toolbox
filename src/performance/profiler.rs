...
impl Profiler {
    ...
    pub async fn record_return(&self, session: &mut ProfileSession) {
        ...
        if let Some(call_info) = session.call_stack.pop() {
            ...
            let duration =
                Duration::from_millis(current_time.saturating_sub(call_info.start_time_millis));
    ...
    pub async fn get_report(&self, component: &str) -> Option<ProfilingReport> {
        ...
        let mut hotspots: Vec<_> = function_stats
            .iter()
        ...
        Some(ProfilingReport {
            ...
            hotspots: hotspots
                .into_iter()
                .map(|(name, duration)| Hotspot { name, duration })
                .collect(),
        ...
        for (i, call) in call_stack.iter().enumerate() {
            ...
            if i + 1 < call_stack.len() {
                ...
                if let Some(edge) = self
                    .edges
                    .iter_mut()
                    .find(|e| e.from == *caller && e.to == *callee)
                {
                    edge.call_count += 1;
                } else {
...
mod tests {
    ...
    async fn test_flame_graph() {
        ...
        assert_eq!(
            flame_graph.stacks["process_data"],
