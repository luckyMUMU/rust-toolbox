...
### 任务1.1: 删除旧trait系统
- [x] 删除 `ToolRegistry` trait
- [x] 删除 `ToolNode` trait  
- [x] 删除 `ToolExecutor` trait
- [x] 删除 `ComposableTool` trait
- [x] 清理 `tools/registry.rs` 中的dyn代码
- [x] 清理 `tools/node.rs` 中的dyn代码
- [x] 删除 `async_trait` 依赖（如果仅用于工具系统）

**验收标准**:
- [x] `cargo check` 通过（编译错误预期，因为新系统未实现）
- [x] 删除约2000行旧trait代码
...
### 任务1.2: 创建枚举类型系统
- [x] 创建 `src/tools/types.rs` 定义Tool枚举
- [x] 定义 `Tool` 枚举包含所有工具类型变体
- [x] 定义 `ToolKind` 区分工具类别
- [x] 为 `Tool` 实现统一执行接口
- [x] 创建 `ToolId` 类型安全标识符
...
### 任务1.3: 重构工具注册表
- [x] 创建 `src/tools/registry/mod.rs` 新模块
- [x] 实现 `ToolRegistry` 结构体（具体类型，非trait）
...
### 任务2.1: 设计中间件trait
- [x] 创建 `src/tools/middleware/mod.rs`
- [x] 定义 `ToolMiddleware` trait
- [x] 定义 `Next` 类型（中间件链 continuation）
- [x] 设计中间件上下文 `MiddlewareContext`
...
#### 2.2.1 日志中间件
- [x] 创建 `middleware/logging.rs`
- [x] 实现请求/响应日志记录
- [x] 支持结构化日志（JSON）
- [x] 支持日志级别配置

#### 2.2.2 缓存中间件
- [x] 创建 `middleware/cache.rs`
- [x] 实现结果缓存
- [x] 支持TTL
- [x] 支持缓存键自定义
- [x] 支持缓存失效策略
...
#### 2.2.4 超时中间件
- [x] 创建 `middleware/timeout.rs`
- [x] 实现超时控制
- [x] 支持优雅取消
- [x] 支持超时自定义

#### 2.2.5 指标中间件
- [x] 创建 `middleware/metrics.rs`
- [x] 实现执行时间统计
- [x] 实现成功率统计
- [x] 集成Prometheus导出

#### 2.2.6 验证中间件
- [x] 创建 `middleware/validation.rs`
- [x] 实现参数预验证
- [x] 实现结果后验证
...
### 任务3.1: 设计参数trait
- [x] 创建 `src/tools/params/mod.rs`
- [x] 定义 `ToolInput` trait
- [x] 定义 `ToolOutput` trait
- [x] 定义参数验证trait
- [x] 设计派生宏接口
...
### 任务3.2: 实现派生宏
- [x] 创建 `tool-macros` crate（或内嵌proc-macro）
- [x] 实现 `#[derive(ToolInput)]`
- [x] 实现 `#[param(...)]` 属性宏
- [x] 自动生成JSON Schema
- [x] 自动生成验证代码
...
### 任务3.3: 重构工具为泛型
- [x] 创建 `TypedTool<I: ToolInput, O: ToolOutput>` 类型
- [x] 实现泛型工具构建器
- [x] 重构5个核心工具使用泛型
- [x] 更新工作流执行器支持泛型工具
...
### 任务4.2: 工具脚手架CLI
- [x] 创建 `cargo tool-new` 子命令
- [x] 生成交互式工具模板
- [x] 支持多种工具类型（Native/Python/Docker）
- [x] 生成测试模板
- [x] 生成文档模板
...
### 任务4.3: 工具文档生成
- [x] 从ToolInput/ToolOutput派生自动生成文档
- [x] 生成OpenAPI兼容的schema
- [x] 生成Markdown文档
- [x] 支持示例代码嵌入
...
### 任务5.1: 性能基准测试
- [x] 创建 `benches/tool_execution.rs`
- [x] 测试工具注册性能
- [x] 测试工具执行性能
- [x] 对比新旧系统性能
- [x] 测试中间件开销
