...
use workflow_toolkit::interfaces::tui::widget::{
    SizeConstraints, Widget, WidgetCapabilities, WidgetContext, WidgetError, WidgetId, WidgetState,
};
use workflow_toolkit::interfaces::tui::{
    widgets::{
        LogViewerWidget, PluginManagerWidget, SystemStatusWidget, ToolManagerWidget,
        WorkflowListWidget,
    },
    Theme,
};
...
pub async fn run_comprehensive_verification() -> VerificationResults {
    ...
    let plugin_manager_result =
        verify_widget(PluginManagerWidget::new(), "PluginManagerWidget").await;
    ...
    println!(
        "WorkflowListWidget: {}",
        if workflow_list_result.success {
            "✅ PASS"
        } else {
            "❌ FAIL"
        }
    ...
    println!(
        "ToolManagerWidget: {}",
        if tool_manager_result.success {
            "✅ PASS"
        } else {
            "❌ FAIL"
        }
    ...
    println!(
        "PluginManagerWidget: {}",
        if plugin_manager_result.success {
            "✅ PASS"
        } else {
            "❌ FAIL"
        }
    ...
    println!(
        "SystemStatusWidget: {}",
        if system_status_result.success {
            "✅ PASS"
        } else {
            "❌ FAIL"
        }
...
mod verification_tests {
    ...
    async fn test_comprehensive_widget_verification() -> Result<()> {
        ...
        assert!(
            functional_count >= 3,
            "At least 3 widgets should be functional"
        );
