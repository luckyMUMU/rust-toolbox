...
async fn benchmark_register_n_tools(n: usize) {
    ...
    println!("  Throughput: {:.0} tools/sec", n as f64 / avg_duration.as_secs_f64());
}
...
async fn benchmark_lookup() {
    ...
    println!("  Throughput: {:.0} lookups/sec", iterations as f64 / duration.as_secs_f64());
...
async fn benchmark_execution() {
    ...
    println!("  Throughput: {:.0} execs/sec", iterations as f64 / duration.as_secs_f64());
}
...
async fn benchmark_middleware() {
    ...
    println!("  With logging: {:?} ({:.1}% overhead)", 
        logging_duration,
        ((logging_duration.as_secs_f64() / baseline_duration.as_secs_f64()) - 1.0) * 100.0
    );
    println!("  With timing: {:?} ({:.1}% overhead)",
        timing_duration,
        ((timing_duration.as_secs_f64() / baseline_duration.as_secs_f64()) - 1.0) * 100.0
    );
}

async fn execute_without_middleware(input: ToolInput) -> Result<ToolOutput, workflow_toolkit::error::WorkflowError> {
...
async fn benchmark_comparison() {
    ...
    let improvement = ((old_lookup_time.as_secs_f64() / new_lookup_time.as_secs_f64()) - 1.0) * 100.0;
...
async fn benchmark_old_system_lookup() -> Duration {
    ...
    for i in 0..1000 {
        tools.push((format!("tool_{}", i), create_benchmark_tool(&format!("tool_{}", i))));
    }
