...
use workflow_toolkit::interfaces::tui::{
    Widget, WidgetId, Theme, SharedAppState, MainTuiInterface, TuiApp,
    widgets::{
...
mod end_to_end_tests {
    ...
    async fn test_widget_lifecycle_management() -> Result<()> {
        ...
        let widget_types = vec![
            ("WorkflowList", Box::new(WorkflowListWidget::new()) as Box<dyn Widget>),
            ("LogViewer", Box::new(LogViewerWidget::new()) as Box<dyn Widget>),
            ("ToolManager", Box::new(ToolManagerWidget::new()) as Box<dyn Widget>),
            ("PluginManager", Box::new(PluginManagerWidget::new()) as Box<dyn Widget>),
            ("SystemStatus", Box::new(SystemStatusWidget::new()) as Box<dyn Widget>),
        ];
...
mod cross_component_tests {
    ...
    async fn test_state_synchronization_between_widgets() -> Result<()> {
        ...
        fixture.shared_state.update_workflow_status("Test Workflow 1", WorkflowStatus::Running).await?;
        ...
        let workflows = fixture.shared_state.get_workflows().await;
        let updated_workflow = workflows.iter().find(|w| w.name == "Test Workflow 1").unwrap();
...
mod data_flow_tests {
    ...
    async fn test_shared_state_data_flow() -> Result<()> {
        ...
        fixture.shared_state.update_workflow_status("Test Workflow 1", WorkflowStatus::Completed).await?;
        
        let updated_workflows = fixture.shared_state.get_workflows().await;
        let updated_workflow = updated_workflows.iter().find(|w| w.name == "Test Workflow 1").unwrap();
        ...
        fixture.shared_state.remove_workflow("Test Workflow 2").await?;
        
        let remaining_workflows = fixture.shared_state.get_workflows().await;
        assert_eq!(remaining_workflows.len(), 2);
        assert!(!remaining_workflows.iter().any(|w| w.name == "Test Workflow 2"));
...
mod compatibility_tests {
    ...
    async fn test_terminal_size_handling() -> Result<()> {
        ...
        for (width, height) in terminal_sizes {
            ...
            assert!(render_result.is_ok(), "Rendering should work at {}x{}", width, height);
        }
