...
use workflow_toolkit::{
    ...
    Config, ExecutionStatus, Result, ToolRegistry, WorkflowConfig, WorkflowDefinition,
...
impl IntegrationTestFixture {
    ...
    async fn register_test_tools(registry: &mut BasicToolRegistry) -> Result<()> {
        // Echo tool
        let echo_executor = Arc::new(AsyncFunctionExecutor::new(
        ...
        let echo_tool = BasicTool::builder()
            ...
            .build()
            .map_err(|e| {
                WorkflowError::workflow_execution(&format!("Failed to create echo tool: {}", e))
            })?;

        registry.register_tool(Arc::new(echo_tool))?;

        // Math tool
        let math_executor = Arc::new(AsyncFunctionExecutor::new(
            |params: Value, _context| async move {
                let a = params
                    .get("a")
                ...
                let result = match operation {
                    ...
                    "divide" => {
                        if b != 0.0 {
                            ...
                            return Err(WorkflowError::workflow_execution("Division by zero"));
                        }
                    }
        ...
        let math_tool = BasicTool::builder()
            ...
            .build()
            .map_err(|e| {
                WorkflowError::workflow_execution(&format!("Failed to create math tool: {}", e))
            })?;
...
async fn test_basic_workflow_execution() -> Result<()> {
    ...
    let workflow_def = WorkflowDefinition {
        ...
        nodes: vec![workflow_toolkit::workflow::WorkflowNode {
...
mod performance_tests {
    ...
    async fn benchmark_workflow_execution_throughput() -> Result<()> {
        ...
        for i in 0..num_workflows {
            ...
            let handle: tokio::task::JoinHandle<Result<ExecutionStatus>> =
        ...
        println!(
            "Workflow execution throughput: {:.2} workflows/second",
            throughput
        );
