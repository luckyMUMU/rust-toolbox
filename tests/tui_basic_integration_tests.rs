...
use workflow_toolkit::interfaces::tui::{
    SharedAppState, 
    state::{StateChangeEvent, SystemStatus, SystemHealth, NetworkStatus, ConnectionStatus},
...
mod shared_state_tests {
    ...
    async fn test_state_change_events() -> Result<()> {
        ...
        fixture.shared_state.add_workflow(new_workflow.clone()).await?;
        ...
        fixture.shared_state.update_workflow_status("Event Test Workflow", WorkflowStatus::Running).await?;
    ...
    async fn test_concurrent_state_access() -> Result<()> {
        ...
        for handle in handles {
            handle.await.map_err(|e| {
                workflow_toolkit::error::WorkflowError::ValidationError(
        ...
        for i in 0..5 {
            assert!(workflows.iter().any(|w| w.name == format!("Concurrent Workflow {}", i)));
    ...
    async fn test_data_persistence_and_retrieval() -> Result<()> {
        ...
        fixture.shared_state.remove_workflow("Persistence Test Workflow 1").await?;
        
        let final_workflows = fixture.shared_state.get_workflows().await;
        assert_eq!(final_workflows.len(), 4);
        assert!(!final_workflows.iter().any(|w| w.name == "Persistence Test Workflow 1"));
    ...
    async fn test_connection_status_management() -> Result<()> {
        ...
        // Test status changes
        fixture.shared_state.set_connection_status(ConnectionStatus::Connecting).await?;
        ...
        fixture.shared_state.set_connection_status(ConnectionStatus::Connected).await?;
        ...
        // Test error status
        fixture.shared_state.set_connection_status(ConnectionStatus::Error("Test error".to_string())).await?;
        let error_status = fixture.shared_state.get_connection_status().await;
        assert_eq!(error_status, ConnectionStatus::Error("Test error".to_string()));
    ...
    async fn test_performance_metrics_tracking() -> Result<()> {
        ...
        fixture.shared_state.record_widget_update("workflow_list").await?;
        fixture.shared_state.record_widget_update("log_viewer").await?;
        fixture.shared_state.record_widget_update("workflow_list").await?;
        fixture.shared_state.record_widget_update("system_status").await?;
        
        let updated_metrics = fixture.shared_state.get_metrics().await;
        assert_eq!(updated_metrics.widget_update_count.get("workflow_list"), Some(&2));
        assert_eq!(updated_metrics.widget_update_count.get("log_viewer"), Some(&1));
        assert_eq!(updated_metrics.widget_update_count.get("system_status"), Some(&1));
        ...
        fixture.shared_state.update_metrics(new_metrics.clone()).await?;
    ...
    async fn test_data_staleness_detection() -> Result<()> {
        ...
        let is_stale = fixture.shared_state.is_data_stale(chrono::Duration::seconds(1)).await;
        assert!(!is_stale);
        ...
        let is_stale_short = fixture.shared_state.is_data_stale(chrono::Duration::milliseconds(5)).await;
        ...
        let is_stale_after_refresh = fixture.shared_state.is_data_stale(chrono::Duration::seconds(1)).await;
...
mod error_handling_tests {
    ...
    async fn test_invalid_operations() -> Result<()> {
        ...
        let result = fixture.shared_state.update_workflow_status("NonExistent", WorkflowStatus::Running).await;
    ...
    async fn test_unicode_data_handling() -> Result<()> {
        ...
        assert!(workflows[0].description.as_ref().unwrap().contains("✨🔧⚡"));
...
mod performance_tests {
    ...
    async fn test_large_dataset_performance() -> Result<()> {
        ...
        for i in 0..workflow_count {
            let workflow = WorkflowInfo {
                ...
                status: if i % 3 == 0 { WorkflowStatus::Running } else { WorkflowStatus::Available },
        ...
        for i in 0..log_count {
            let log = LogEntry {
                ...
                execution_id: if i % 5 == 0 { Some(format!("exec-{}", i / 5)) } else { None },
                workflow_id: if i % 3 == 0 { Some(format!("workflow-{}", i / 3)) } else { None },
                node_id: if i % 7 == 0 { Some(format!("node-{}", i / 7)) } else { None },
            };
    ...
    async fn test_concurrent_access_performance() -> Result<()> {
            ...
            let handle = tokio::spawn(async move {
                for i in 0..operations_per_task {
                    ...
                    let workflow = WorkflowInfo {
                        ...
                        version: "1.0.0".to_string(),
                        description: Some(format!("Workflow from task {} operation {}", task_id, i)),
                        status: WorkflowStatus::Available,
                        last_execution: None,
                        execution_count: 0,
        ...
        println!("Completed {} concurrent operations in {:?}", total_operations, elapsed);
