...
use workflow_toolkit::tools::{
    Tool, ToolId, ToolInput, ToolOutput, ToolMetadata, ToolKind,
    ToolRegistry, ToolRegistryBuilder,
    LoggingMiddleware, TimingMiddleware, RetryMiddleware, TimeoutMiddleware,
    CircuitBreakerMiddleware, MetricsMiddleware, CacheMiddleware,
    MiddlewareStack, MiddlewareStackBuilder,
    ToolChain, ConditionalTool, ParallelTools, ToolCompositionBuilder,
};
...
async fn test_registry_list() {
    ...
    registry.register("tool1", create_test_tool("tool1")).await.unwrap();
    registry.register("tool2", create_test_tool("tool2")).await.unwrap();
    registry.register("tool3", create_test_tool("tool3")).await.unwrap();
...
async fn test_logging_middleware() {
    ...
    let result = middleware.execute(ctx, input, |_, _| {
...
async fn test_circuit_breaker_middleware() {
    ...
    for _ in 0..2 {
        let _ = middleware.execute(ctx.clone(), input.clone(), |_, _| {
            Box::pin(async {
                Err(WorkflowError::ExecutionError("Fail".to_string()))
            })
        }).await;
    }
...
async fn test_middleware_chain() {
    ...
    let result = stack.execute(ctx, input, |_, _| {
        Box::pin(async { Ok(ToolOutput::success(json!({}))) })
    }).await;
    ...
    impl ToolInputConvert for TestInput {
        ...
        fn validate(&self) -> Result<(), WorkflowError> {
            if self.message.is_empty() {
                return Err(WorkflowError::ValidationError("Message cannot be empty".to_string()));
            }
            Ok(())
        }
...
fn test_composition_builder() {
    ...
    builder.chain("chain1", "First chain")
        .add_step("step1", ToolId::new("tool1"))
        .register();
    
    builder.conditional("conditional1", "First conditional", "x > 0")
        .then(ToolId::new("then_tool"))
        .otherwise(ToolId::new("else_tool"))
        .register()
        .unwrap();
    
    builder.parallel("parallel1", "First parallel")
