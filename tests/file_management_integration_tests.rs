...
use workflow_toolkit::{
    Config, WorkflowDefinition, WorkflowEngine, ExecutionStatus,
    ToolRegistry, Result, WorkflowError, WorkflowConfig,
    config::ConfigManager,
    tools::{BasicToolRegistry, BasicTool, AsyncFunctionExecutor},
    storage::{SimpleMemoryCache, FileStorage, StateManager},
    workflow::{engine::DefaultWorkflowEngine, WorkflowState, WorkflowNode, NodeType},
    ExecutionContext,
};
...
impl FileManagementTestFixture {
    ...
    async fn register_mock_file_management_tools(registry: &mut BasicToolRegistry) -> Result<()> {
        ...
        let classifier_executor = Arc::new(AsyncFunctionExecutor::new(|params: Value, _context| async move {
            let folder_path = params.get("folder_path").and_then(|v| v.as_str()).unwrap_or("");
            let experimental_mode = params.get("experimental_mode").and_then(|v| v.as_bool()).unwrap_or(false);
            
            // Mock classification logic
            let category = if folder_path.contains("document") || folder_path.contains("文档") || folder_path.contains("Documents") || folder_path.contains("Project") {
                "documents"
            } else if folder_path.contains("photo") || folder_path.contains("music") || folder_path.contains("Photos") || folder_path.contains("Music") {
                "media"
            } else {
                "other"
            };
        ...
        let classifier_tool = BasicTool::builder()
            ...
            .build()
            .map_err(|e| WorkflowError::workflow_execution(&format!("Failed to create classifier tool: {}", e)))?;
        
        registry.register_tool(Arc::new(classifier_tool))?;
        
        // Mock text processor
        let text_processor_executor = Arc::new(AsyncFunctionExecutor::new(|params: Value, _context| async move {
            let text = params.get("text").and_then(|v| v.as_str()).unwrap_or("");
            let empty_vec = vec![];
            let operations = params.get("operations").and_then(|v| v.as_array()).unwrap_or(&empty_vec);
        ...
        let text_processor_tool = BasicTool::builder()
            ...
            .build()
            .map_err(|e| WorkflowError::workflow_execution(&format!("Failed to create text processor tool: {}", e)))?;
        
        registry.register_tool(Arc::new(text_processor_tool))?;
        
        // Mock batch processor
        let batch_processor_executor = Arc::new(AsyncFunctionExecutor::new(|params: Value, _context| async move {
            let empty_vec = vec![];
            let items = params.get("items").and_then(|v| v.as_array()).unwrap_or(&empty_vec);
            ...
            for (i, item) in items.iter().enumerate() {
                ...
                if tool_name == "folder-classifier" {
                    let folder_path = item.get("folder_path").and_then(|v| v.as_str()).unwrap_or("");
        ...
        let batch_processor_tool = BasicTool::builder()
            ...
            .build()
            .map_err(|e| WorkflowError::workflow_execution(&format!("Failed to create batch processor tool: {}", e)))?;
        
        registry.register_tool(Arc::new(batch_processor_tool))?;
        
        // Mock human decision tool
        let human_decision_executor = Arc::new(AsyncFunctionExecutor::new(|params: Value, _context| async move {
            let _decision_type = params.get("decision_type").and_then(|v| v.as_str()).unwrap_or("Custom");
            let empty_vec = vec![];
            let options = params.get("options").and_then(|v| v.as_array()).unwrap_or(&empty_vec);
            
            // Auto-select recommended option or first option
            let recommended_option = options.iter()
                .find(|opt| opt.get("recommended").and_then(|v| v.as_bool()).unwrap_or(false))
                .or_else(|| options.first());
            
            if let Some(option) = recommended_option {
                let selected_id = option.get("id").and_then(|v| v.as_str()).unwrap_or("default");
                return Ok(json!({
        ...
        let human_decision_tool = BasicTool::builder()
            ...
            .build()
            .map_err(|e| WorkflowError::workflow_execution(&format!("Failed to create human decision tool: {}", e)))?;
    ...
    async fn register_test_helper_tools(registry: &mut BasicToolRegistry) -> Result<()> {
        ...
        let scanner_executor = Arc::new(AsyncFunctionExecutor::new(|params: Value, _context| async move {
        ...
        let scanner_tool = BasicTool::builder()
            ...
            .build()
            .map_err(|e| WorkflowError::workflow_execution(&format!("Failed to create scanner tool: {}", e)))?;
        
        registry.register_tool(Arc::new(scanner_tool))?;
        
        // Rule validator mock
        let validator_executor = Arc::new(AsyncFunctionExecutor::new(|params: Value, _context| async move {
        ...
        let validator_tool = BasicTool::builder()
            ...
            .build()
            .map_err(|e| WorkflowError::workflow_execution(&format!("Failed to create validator tool: {}", e)))?;
...
async fn test_classification_tool_integration() -> Result<()> {
    ...
    assert_eq!(result.get("category").and_then(|v| v.as_str()), Some("documents"));
    
    println!("Classification result: {}", serde_json::to_string_pretty(&result)?);
...
async fn test_text_processor_tool_integration() -> Result<()> {
    ...
    let pinyin_variants = result.get("pinyin_variants").and_then(|v| v.as_array()).unwrap();
...
async fn test_batch_processor_integration() -> Result<()> {
    ...
    let batch_items: Vec<Value> = test_folders.iter().map(|path| {
        json!({
    ...
    let total_items = result.get("total_items").and_then(|v| v.as_u64()).unwrap_or(0);
    assert_eq!(total_items, test_folders.len() as u64);
    
    println!("Batch processing result: {}", serde_json::to_string_pretty(&result)?);
...
async fn test_human_decision_experimental_mode() -> Result<()> {
    ...
    let selected = result.get("selected_option").and_then(|v| v.as_str()).unwrap_or("");
    assert_eq!(selected, "documents"); // Should auto-select recommended option
    
    println!("Human decision result (experimental): {}", serde_json::to_string_pretty(&result)?);
...
async fn test_tool_interaction_data_flow() -> Result<()> {
    ...
    let text_result = fixture.tool_registry.execute_tool(
        "text-processor",
        json!({
    ...
    let processed_text = text_result.get("processed").and_then(|v| v.as_str()).unwrap_or("");
    let pinyin_variants = text_result.get("pinyin_variants").cloned().unwrap_or(json!([]));
    ...
    let classification_result = fixture.tool_registry.execute_tool(
        "folder-classifier",
        json!({
    ...
    assert_eq!(classification_result.get("category").and_then(|v| v.as_str()), Some("documents"));
    
    println!("Text processing -> Classification data flow test completed");
    println!("Processed text: {}", processed_text);
    println!("Classification result: {}", serde_json::to_string_pretty(&classification_result)?);
...
mod performance_tests {
    ...
    async fn test_batch_processing_performance_with_profiling() -> Result<()> {
        ...
        let monitor = fixture.performance_manager.start_monitoring("batch_processing_test").await;
        
        // Create a large number of test items for stress testing
        let num_items = 100;
        let batch_items: Vec<Value> = (0..num_items).map(|i| {
            json!({
        ...
        let result = fixture.base_fixture.tool_registry.execute_tool(
            "batch-processor",
            json!({
                "items": batch_items,
                "tool_name": "folder-classifier",
        ...
        let processed_items = result.get("processed_items").and_then(|v| v.as_u64()).unwrap_or(0);
        ...
        println!("    Allocated: {} bytes", perf_result.memory_usage.allocated);
        
        // Performance assertions based on requirement 10.4
        assert!(throughput >= 10.0, "Throughput too low: {:.2} items/second", throughput);
        
        // Memory efficiency check - should not allocate more than 10MB for 100 items
        assert!(perf_result.memory_usage.allocated < 10_000_000, 
                "Memory usage too high: {} bytes", perf_result.memory_usage.allocated);
        
        // Get and verify performance statistics
        let stats = fixture.performance_manager.get_stats("batch_processing_test").await;
    ...
    async fn test_text_processing_performance_with_profiling() -> Result<()> {
        ...
        for (i, text) in test_texts.iter().enumerate() {
            let monitor = fixture.performance_manager.start_monitoring(&format!("text_processing_{}", i)).await;
            ...
            println!("  Duration: {:?}", elapsed);
            println!("  Memory allocated: {} bytes", perf_result.memory_usage.allocated);
        }
        ...
        println!("  Average memory per operation: {} bytes", avg_memory_per_operation);
        
        // Performance assertions based on requirement 10.4
        assert!(avg_latency < Duration::from_millis(50), "Average latency too high: {:?}", avg_latency);
        
        // Memory efficiency check - should not allocate more than 1MB per operation on average
        assert!(avg_memory_per_operation < 1_000_000, 
                "Average memory usage too high: {} bytes per operation", avg_memory_per_operation);
    ...
    async fn test_memory_usage_under_load() -> Result<()> {
        ...
        for load_size in load_sizes {
            let monitor = fixture.performance_manager.start_monitoring(&format!("memory_load_test_{}", load_size)).await;
            ...
            println!("  Memory allocated: {} bytes", perf_result.memory_usage.allocated);
            println!("  Peak memory: {} bytes", perf_result.memory_usage.peak_usage);
            println!("  Memory per item: {} bytes", 
                     if processed_items > 0 { perf_result.memory_usage.allocated / processed_items as usize } else { 0 });
        }
        
        // Analyze memory scaling
        println!("Memory scaling analysis:");
        for (load_size, memory_usage) in &memory_usage_results {
            println!("  Load {}: {} bytes allocated, {} bytes peak", 
        ...
        if memory_usage_results.len() >= 2 {
            ...
            if first.1.allocated == 0 && last.1.allocated == 0 {
                ...
                assert!(memory_ratio <= load_ratio * 3.0, 
                        "Memory scaling too high: {:.2}x vs load {:.2}x", memory_ratio, load_ratio);
            }
        }
    ...
    async fn test_concurrent_processing_efficiency() -> Result<()> {
        ...
        for concurrency in concurrency_levels {
            let monitor = fixture.performance_manager.start_monitoring(&format!("concurrency_test_{}", concurrency)).await;
            ...
            results.push((concurrency, elapsed, throughput, perf_result.memory_usage.allocated));
        ...
        for (concurrency, duration, throughput, memory) in &results {
            ...
            println!("  Concurrency {}: {:.2}x speedup, {:.2}% efficiency, {} bytes memory", 
                     concurrency, speedup, efficiency * 100.0, memory);
            
            // Efficiency should be reasonable (at least 20% for higher concurrency in mock environment)
            if *concurrency <= 4 {
                assert!(efficiency >= 0.2, "Concurrency efficiency too low: {:.2}%", efficiency * 100.0);
            }
        }
    ...
    async fn test_resource_cleanup_and_monitoring() -> Result<()> {
        ...
        for i in 0..5 {
            let monitor = fixture.performance_manager.start_monitoring(&format!("cleanup_test_{}", i)).await;
            
            let _result = fixture.base_fixture.tool_registry.execute_tool(
                "folder-classifier",
                json!({
        ...
        println!("  New stats recorded: {}", final_stats.len() - initial_count);
        
        // Should have recorded stats for each operation
        assert!(final_stats.len() >= initial_count + 5, 
        ...
        for (component, stats) in &final_stats {
            if component.starts_with("cleanup_test_") {
                assert!(stats.execution_count > 0, "Execution count should be recorded");
                assert!(stats.total_duration > Duration::ZERO, "Duration should be recorded");
                assert!(stats.last_updated_timestamp > 0, "Timestamp should be recorded");
                
                println!("  Component {}: {} executions, {:?} total duration", 
...
mod error_handling_tests {
    ...
    async fn test_batch_processing_partial_failures() -> Result<()> {
        ...
        let processed = result.get("processed_items").and_then(|v| v.as_u64()).unwrap_or(0);
