...
impl WorkflowEngineContractTests {
    ...
    /// 测试：暂停工作流应成功
    pub async fn test_pause_workflow<E: WorkflowEngineContract>(engine: &E) -> Result<(), String> {
        let params = HashMap::new();
    ...
    /// 测试：恢复工作流应成功
    pub async fn test_resume_workflow<E: WorkflowEngineContract>(engine: &E) -> Result<(), String> {
        let params = HashMap::new();
    ...
    /// 测试：停止工作流应成功
    pub async fn test_stop_workflow<E: WorkflowEngineContract>(engine: &E) -> Result<(), String> {
        let params = HashMap::new();
    ...
    /// 运行所有契约测试
    pub async fn run_all<E: WorkflowEngineContract>(
        engine: &E,
    ) -> Vec<(String, Result<(), String>)> {
        vec![
            (
                "execute_returns_execution_id".to_string(),
                Self::test_execute_returns_execution_id(engine).await,
            ),
            (
                "execute_status_is_valid".to_string(),
                Self::test_execute_status_is_valid(engine).await,
            ),
            (
                "pause_workflow".to_string(),
                Self::test_pause_workflow(engine).await,
            ),
            (
                "resume_workflow".to_string(),
                Self::test_resume_workflow(engine).await,
            ),
            (
                "stop_workflow".to_string(),
                Self::test_stop_workflow(engine).await,
            ),
        ]
    }
}
