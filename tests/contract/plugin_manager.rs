...
pub trait PluginManagerContract: Send + Sync {
    ...
    /// 关闭所有插件
    fn shutdown_all(&self) -> impl std::future::Future<Output = Result<(), Self::Error>> + Send;
}
...
impl PluginManagerContractTests {
    ...
    /// 测试：获取不存在的插件应返回 None
    pub fn test_get_nonexistent<M: PluginManagerContract>(manager: &M) -> Result<(), String> {
    ...
    pub async fn run_all<M: PluginManagerContract>(
        ...
        vec![
            (
                ...
                Ok(Self::test_get_nonexistent(manager)),
            ),
            (
                "load_and_is_loaded".to_string(),
                Self::test_load_and_is_loaded(manager, config.clone()).await,
            ),
            (
                "list_plugins".to_string(),
                Self::test_list_plugins(manager, config.clone()).await,
            ),
            (
                "plugin_count".to_string(),
                Self::test_plugin_count(manager, config.clone()).await,
            ),
            (
                "unload".to_string(),
                Self::test_unload(manager, config.clone()).await,
            ),
            (
                "shutdown_all".to_string(),
                Self::test_shutdown_all(manager, config).await,
            ),
        ]
    }
}
