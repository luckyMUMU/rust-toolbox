...
impl ToolRegistryContractTests {
    ...
    /// 测试：获取不存在的工具应返回 None
    pub fn test_get_nonexistent<R: ToolRegistryContract>(
        registry: &R,
    ) -> Result<(), String> {
    ...
    pub fn run_all<R: ToolRegistryContract>(
        ...
        vec![
            ("register_and_contains".to_string(), Self::test_register_and_contains(registry, tool.clone())),
            ("unregister".to_string(), Self::test_unregister(registry, tool.clone())),
            ("get_nonexistent".to_string(), Self::test_get_nonexistent(registry)),
            ("list_tools".to_string(), Self::test_list_tools(registry, tool)),
        ]
    }
}
...
mod tests {
    ...
    impl ToolRegistryContract for MockToolRegistry {
        ...
        fn register(&self, name: &str, tool: impl Send + Sync + 'static) -> Result<(), Self::Error> {
            self.tools.write().unwrap().insert(name.to_string(), Arc::new(tool));
            Ok(())
        }
