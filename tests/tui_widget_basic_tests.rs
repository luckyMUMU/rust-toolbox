...
use workflow_toolkit::interfaces::tui::widget::{
    Widget, WidgetId, WidgetContext, WidgetCapabilities, WidgetState, WidgetError
};
...
impl Widget for MockWidget {
    ...
    async fn render(&mut self, frame: &mut ratatui::Frame, area: Rect, theme: &Theme) -> std::result::Result<(), WidgetError> {
        ...
        let paragraph = Paragraph::new("Mock Widget Content")
            .block(block);
    ...
    async fn handle_event(&mut self, _event: Event) -> std::result::Result<Option<workflow_toolkit::interfaces::tui::action::Action>, WidgetError> {
        Ok(None)
    }
}
