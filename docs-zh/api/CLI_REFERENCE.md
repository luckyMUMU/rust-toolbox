...
> **Workflow Toolkit 命令行界面的完整参考手册**  
> *最后更新时间：2026-01-14*

## 概览

Workflow Toolkit CLI (`workflow-toolkit`) 提供了一个统一的界面，用于管理工作流、工具、插件和系统服务。
...
- `--params-json <JSON>`: 以 JSON 字符串形式提供的参数
- `--background`: 在后台执行
- `--wait`: 等待完成并显示进度
- `--timeout <秒>`: 执行超时时间
- `--dry-run`: 预览而不执行
...
**选项：**
- `--status <状态>`: 按执行状态过滤 (Pending, Running 等)
- `--recent`: 仅显示最近的执行
- `--limit <数量>`: 最大结果数量 (默认: 50)
...
**选项：**
- `--params <JSON>`: 以 JSON 字符串形式提供的工具参数
...
### `tui`
启动终端用户界面 (Terminal User Interface)。
...
- `--ws-port <端口>`: WebSocket 端口 (默认: 8081)
- `--auth`: 启用身份验证
