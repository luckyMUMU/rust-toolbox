...
use workflow_toolkit::{
    ...
    ToolRegistry, workflow::engine::DefaultWorkflowEngine,
    ...
    workflow::{WorkflowNode, NodeType, WorkflowEdge, RetryPolicy, BackoffStrategy},
    ExecutionContext,
};
...
async fn setup_workflow_components() -> Result<WorkflowComponents> {
    let temp_dir = tempfile::TempDir::new()
        .map_err(|e| WorkflowError::workflow_execution(&format!("Failed to create temp dir: {}", e)))?;
    
    // Configuration
    let config = Arc::new(ConfigManager::new(Config::default()));
    
    // Storage
    let storage = Arc::new(FileStorage::new(&temp_dir.path().join("storage"))
        .map_err(|e| WorkflowError::workflow_execution(&format!("Failed to create storage: {}", e)))?);
            ...
            match engine.get_workflow_status(execution.id).await {
                Ok(status) => {
                    if status != last_status {
                        println!("  📊 状态变更: {:?} -> {:?} (检查 #{})", last_status, status, check_count);
...
fn create_data_processor_tool() -> Result<BasicTool> {
    let executor = Arc::new(AsyncFunctionExecutor::new(|params: Value, _context: ExecutionContext| async move {
        // Simulate data processing
        tokio::time::sleep(Duration::from_millis(100)).await;
        
        let data = params.get("data").cloned().unwrap_or(json!([]));
        let operations = params.get("operations").and_then(|v| v.as_array()).unwrap_or(&vec![]);
...
fn create_file_operations_tool() -> Result<BasicTool> {
    let executor = Arc::new(AsyncFunctionExecutor::new(|params: Value, _context: ExecutionContext| async move {
        ...
        match operation {
            ...
            _ => Err(WorkflowError::tool_execution(&format!("Unknown operation: {}", operation)))
        }
    }));
    ...
    let executor = Arc::new(AsyncFunctionExecutor::new(|params: Value, _context: ExecutionContext| async move {
        ...
        match operation {
            ...
            "power" => {
                ...
                let exponent = params.get("exponent").and_then(|v| v.as_f64()).unwrap_or(8.0);
...
fn create_notification_tool() -> Result<BasicTool> {
    let executor = Arc::new(AsyncFunctionExecutor::new(|params: Value, _context: ExecutionContext| async move {
        ...
        let channel = params.get("channel").and_then(|v| v.as_str()).unwrap_or("console");
...
fn create_error_simulator_tool() -> Result<BasicTool> {
    let executor = Arc::new(AsyncFunctionExecutor::new(|params: Value, _context: ExecutionContext| async move {
        let error_type = params.get("error_type").and_then(|v| v.as_str()).unwrap_or("temporary");
        let failure_rate = params.get("failure_rate").and_then(|v| v.as_f64()).unwrap_or(0.5);
        let message = params.get("message").and_then(|v| v.as_str()).unwrap_or("Simulated error");
        ...
        if random_value < failure_rate {
            Err(WorkflowError::tool_execution(&format!("{}: {}", error_type, message)))
        } else {
            Ok(json!({
                "success": true,
