...
use workflow_toolkit::config::{Config, ConfigManager, CliConfigOverrides, ConfigPriority, ConfigSource};
use std::sync::Arc;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Initial HTTP port: {}", manager.get_config().server.http_port);
    ...
    println!("After environment override: {}", manager.get_config().server.http_port);
    ...
    println!("After CLI override (highest priority): {}", manager.get_config().server.http_port);
...
"#;
    ...
    println!("Initial config loaded - HTTP port: {}", hot_reload_manager.get_config().server.http_port);
...
"#;
    ...
    println!("Config reloaded - HTTP port: {}", hot_reload_manager.get_config().server.http_port);
    ...
    for source in sources {
        println!("Source: {} (Priority: {:?})", source.source, source.priority);
    }
    ...
    if watch_receiver.changed().await.is_ok() {
        ...
        println!("Watch receiver detected config change - HTTP port: {}", updated_config.server.http_port);
    }
