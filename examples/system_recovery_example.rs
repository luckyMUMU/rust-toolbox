...
use workflow_toolkit::{
    ...
    tools::ToolRegistry,
    workflow::{
        CacheConfig, DefaultWorkflowEngine, InvalidationStrategy, NodeType, WorkflowDefinition,
        WorkflowNode,
    },
};
...
impl ToolRegistry for MockToolRegistry {
    ...
    ) -> workflow_toolkit::error::Result<Value> {
        // Simulate some work
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    workflow.add_edge(workflow_toolkit::workflow::WorkflowEdge::new(
        "fetch_data",
        "process_data",
    ))?;
    workflow.add_edge(workflow_toolkit::workflow::WorkflowEdge::new(
    ...
    if recovered_workflows.is_empty() {
        ...
        println!(
            "   Recovered {} incomplete workflows",
            recovered_workflows.len()
        );
        for recovered in &recovered_workflows {
            println!(
                "     - Workflow: {} (ID: {})",
                recovered.workflow_name, recovered.id
            );
    ...
    let invalidated_count = engine
        .invalidate_workflow_cache("data-processing-pipeline", Some("1.0.0"))
        .await?;
    println!(
