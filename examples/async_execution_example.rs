...
use workflow_toolkit::{
    ...
    ConcurrencyConfig, Config, DefaultExecutionManager, ExecutionContext, ExecutionManager,
    ExecutionMode, ExecutionResult, NodeType, ResourceLimits, TaskPriority, WorkflowDefinition,
    WorkflowNode,
};
...
impl ToolRegistry for ExampleToolRegistry {
    fn register_tool(
        &mut self,
        _tool: Arc<dyn workflow_toolkit::tools::ToolNode>,
    ) -> workflow_toolkit::Result<()> {
        Ok(())
    }
    ...
    ) -> workflow_toolkit::Result<serde_json::Value> {
        // Simulate some work
...
async fn main() -> workflow_toolkit::Result<()> {
    ...
    let sync_result = execution_manager
        .execute_workflow(
            workflow.clone(),
            ExecutionMode::Sync,
    ...
    let async_result = execution_manager
        .execute_workflow_with_priority(
            workflow.clone(),
            ExecutionMode::Async,
            ExecutionContext::new(),
            TaskPriority::High,
    ...
    match async_result {
        ExecutionResult::Async(handle) => {
            println!(
                "✅ Async execution submitted in {:?}",
                async_submit_duration
            );
    ...
    for i in 0..3 {
        let mut concurrent_workflow =
            WorkflowDefinition::new(&format!("concurrent_{}", i), "1.0.0");
