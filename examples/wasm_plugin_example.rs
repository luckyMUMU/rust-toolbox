...
use workflow_toolkit::plugins::{
    Plugin, PluginConfig, WasmPlugin, WasmPluginBuilder, WasmRuntimeConfig,
    WasmRuntimeType, ExtismConfig, ResourceLimits, SecurityPolicy,
};
...
async fn main() -> Result<()> {
    ...
    println!("✅ Wasmtime plugin created: {}", wasmtime_plugin.info().name);
    ...
    let mut extism_plugin = WasmPluginBuilder::new(
        "multi-lang-processor".to_string(),
        "1.0.0".to_string(),
    )
    .with_description("Multi-language WASM plugin using Extism".to_string())
    .with_author("Workflow Toolkit Team".to_string())
    .with_module_path(PathBuf::from("examples/wasm_tools/multi_lang_processor.wasm"))
    ...
    for (mut plugin, config, name) in [
        (Box::new(wasmtime_plugin) as Box<dyn Plugin>, wasmtime_config, "Wasmtime"),
        (Box::new(extism_plugin) as Box<dyn Plugin>, extism_config, "Extism"),
    ] {
        println!("\n🔧 Testing {} plugin...", name);
        ...
        match plugin.initialize(config) {
            ...
            Err(e) => {
                ...
                if name == "Wasmtime" {
                    println!("   To create a Wasmtime-compatible WASM module, compile the .wat file:");
                    println!("   wat2wasm examples/wasm_tools/simple_calculator.wat");
                } else {
                    println!("   To create an Extism-compatible WASM module:");
                    println!("   Use any language that compiles to WASM (Rust, Go, JavaScript, etc.)");
                }
            }
        }
    }
    ...
    println!("  - Memory limit: {} MB", wasmtime_config.memory_limit / (1024 * 1024));
    ...
    println!("\n📋 Extism Configuration:");
    println!("  - Memory limit: {} MB", extism_runtime_config.memory_limit / (1024 * 1024));
    ...
    if let Some(ref extism_config) = extism_runtime_config.extism_config {
        ...
        println!("  - Config data keys: {:?}", extism_config.config_data.keys().collect::<Vec<_>>());
...
fn create_wasm_plugin_examples() -> Vec<Box<dyn Plugin>> {
    vec![
        ...
        Box::new(WasmPluginBuilder::new("calculator-wasmtime".to_string(), "1.0.0".to_string())
        ...
        Box::new(WasmPluginBuilder::new("text-processor-extism".to_string(), "2.1.0".to_string())
        ...
        Box::new(WasmPluginBuilder::new("validator-wasmtime".to_string(), "1.5.0".to_string())
            ...
            .with_runtime_type(WasmRuntimeType::Wasmtime)
            .add_entry_point("validate_json".to_string(), "validate_json_schema".to_string())
            .add_entry_point("validate_email".to_string(), "validate_email_format".to_string())
            .build()),

        // Example 4: Network-enabled processor using Extism
        Box::new(WasmPluginBuilder::new("network-processor".to_string(), "3.0.0".to_string())
            ...
            .add_entry_point("process_response".to_string(), "process_api_response".to_string())
            .build()),
    ]
}
...
mod tests {
    ...
    fn test_wasm_plugin_creation() {
        ...
        let wasmtime_plugin = WasmPluginBuilder::new("test-wasmtime".to_string(), "1.0.0".to_string())
            .with_runtime_type(WasmRuntimeType::Wasmtime)
            .build();
    ...
    async fn test_wasm_runtime_config() {
        ...
        assert!(matches!(wasmtime_config.runtime_type, WasmRuntimeType::Wasmtime));
        ...
        assert!(matches!(extism_config.runtime_type, WasmRuntimeType::Extism));
    ...
    fn test_extism_config() {
        let config = ExtismConfig {
            ...
            config_data: [("key".to_string(), "value".to_string())].iter().cloned().collect(),
            memory_pages: Some(2),
