...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    state_manager
        .save_workflow_state(workflow_id1, workflow_state1.clone())
        .await?;
    state_manager
        .save_workflow_state(workflow_id2, workflow_state2.clone())
        .await?;
    ...
    state_manager
        .save_execution_record(execution_record1)
        .await?;
    state_manager
        .save_execution_record(execution_record2)
        .await?;
    ...
    println!(
        ...
        backup_metadata.execution_record_count
    );
    println!("   - Size: {} bytes", backup_metadata.size_bytes);

    // Verify the backup
    println!("\n🔍 Verifying backup integrity...");
    let verification = state_manager
        .verify_backup(&backup_metadata.backup_id)
        .await?;
    ...
    state_manager
        .save_workflow_state(workflow_id3, workflow_state3)
        .await?;
    ...
    let incremental_backup = state_manager
        .create_incremental_backup(&backup_metadata.backup_id)
        .await?;
    println!("✅ Incremental backup created:");
    println!("   - Backup ID: {}", incremental_backup.backup_id);
    println!(
        "   - Previous backup: {:?}",
        incremental_backup.previous_backup_id
    );
    ...
    let restore_result = state_manager
        .restore_from_backup(&incremental_backup.backup_id)
        .await?;
    println!("✅ Restore completed:");
    println!(
        "   - Restored workflows: {}",
        restore_result.restored_workflows
    );
    println!(
        "   - Restored execution records: {}",
        restore_result.restored_execution_records
    );
    println!(
        "   - Restored system data keys: {}",
        restore_result.restored_system_data_keys
    );
