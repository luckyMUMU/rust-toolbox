...
async fn main() -> Result<()> {
    ...
    let calculator_executor = Arc::new(AsyncFunctionExecutor::new(|params, _context| async move {
        ...
        let result = match operation {
            ...
            _ => return Err(workflow_toolkit::error::WorkflowError::ValidationError(
    ...
    for tool_info in registry.list_tools() {
        println!("- {} v{}: {}", tool_info.name, tool_info.version, tool_info.description);
    ...
    match registry.execute_tool("calculator", add_params, context.clone()).await {
    ...
    match registry.execute_tool("calculator", divide_params, context.clone()).await {
    ...
    match registry.execute_tool("calculator", divide_zero_params, context.clone()).await {
    ...
    match registry.execute_tool("string_manipulator", uppercase_params, context.clone()).await {
    ...
    match registry.execute_tool("string_manipulator", reverse_params, context.clone()).await {
    ...
    match registry.execute_tool("string_manipulator", length_params, context.clone()).await {
